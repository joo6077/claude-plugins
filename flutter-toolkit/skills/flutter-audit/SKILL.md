...
name: flutter-audit
description: >
  코드 품질 감사. quick 모드(단일 에이전트, 빠른 로컬 검토)와 deep 모드(3에이전트 병렬 감사)를 지원한다.
...
3개 전문 에이전트를 병렬로 실행한다 (haiku 모델로 실행). 인자가 없으면 변경된 파일 전체를 대상으로 하며, 경로가 지정되면 해당 경로만 감사한다.
...
Agent 도구를 사용하여 아래 3개 에이전트를 **동시에** 실행한다.
