...
- **Primitive Substitution Gate (E2 · `HAS_DS = true` 일 때만)** — 아래 Step 5/6 템플릿은 `Scaffold` / `AppBar` / `Text` 로 된 **골격 예시**이지 최종 코드가 아니다. 화면 본문에 `references/primitive-substitution-gate.md` §게이트 대상 의 기본 위젯을 넣기 전에 같은 의미의 프로젝트 컴포넌트를 검색하고 **대체 후보 표**를 채워서 남긴다. 대상 목록·검색 명령·면제 목록은 그 파일이 SSOT 다 — 여기서 다시 세지 마라. `AppBar` 도 프로젝트에 공용 앱바 컴포넌트가 있으면 그것을 쓴다 (기존 화면 1 개를 `Read` 로 확인). 실측 REJECT `RE-02` (2026-08-12) 가 정확히 이 빈틈에서 났다
...
- **완료 선언 전 시각 증거 규약 실행 (`/insights` 2026-07-27 Friction #2)** — 화면이 컴파일된 것과 화면이 실제로 렌더링되는 것은 다른 주장이다. 완료 보고 직전에 `references/visual-evidence-protocol.md` 를 실행하고 Visual Evidence Block 을 채운다. 캡처가 불가능하면 `[미검증]` 을 명시하고 멈춰서 말하라 — 추측 금지
