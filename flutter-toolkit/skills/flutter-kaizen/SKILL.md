...
| 스킬 설계 가이드 | `harness/docs/guides/skill-design-guide.md` | `guide` |
...
3. 현재 flutter-toolkit 상태 스캔:
   ...
   - `harness/docs/guides/skill-design-guide.md` 읽기
...
이 카이젠 세션을 시작하기 전과 끝낼 때 모두 `scripts/validate-plugin.py` 를 실행하여 flutter-toolkit 의 7가지 품질 카테고리 상태를 확인한다.

### 실행

```bash
# 세션 시작 시 현재 상태 파악
python3 scripts/validate-plugin.py flutter-toolkit

# 자동 수정 가능한 항목 먼저 (V5 placeholders, V6 code-fence)
python3 scripts/validate-plugin.py flutter-toolkit --fix --check=placeholders,code-fence
...
- **ERROR** (V1~V7 중 실패): 카이젠 Step 4 (ANALYZE) 의 "높음" 레벨에 자동 편입. 이 카이젠 세션에서 반드시 수정.
