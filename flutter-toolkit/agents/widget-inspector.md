...
### 7. 관례 대조 (Convention Match · quick 모드)
...
- 표를 넘겨받았으면 칸마다 `맞음` · `어긋남` · `[미검증]` 가운데 하나와 양쪽 `파일:라인` 을 적는다. 표가 없으면 칸을 적지 않고 아래 두 줄 가운데 하나만 적는다
- 호출 프롬프트가 「관례 표 없는 호출」 이라고 밝혔으면 이 기준을 건너뛰고 리포트 Convention Match 칸에 `건너뜀 — 관례 표 없는 호출` 한 줄만 적는다. 관례 표를 만들지 않는 스킬(flutter-feature)이 부른 경우라 `[미검증]` 이 아니다
- 그런 말 없이 표만 빠졌으면 `[미검증] 관례 표 없음`, 표의 경로를 읽지 못했으면 `[미검증] 경로 확인 실패 — <경로>` 를 적는다
...
Convention Match (관례 대조 · quick)
  ...
  또는 표가 없을 때 한 줄: 건너뜀 — 관례 표 없는 호출 / [미검증] 관례 표 없음
...
- **MUST** 관례 대조는 호출 스킬이 넘긴 관례 표의 경로만 읽는다 — 대조할 화면 수를 새로 정하거나 표 밖 화면을 찾지 않는다. 호출 프롬프트가 「관례 표 없는 호출」 이라고 밝혔으면 `건너뜀 — 관례 표 없는 호출` 을, 그런 말 없이 표만 없으면 `[미검증] 관례 표 없음` 을 적는다
