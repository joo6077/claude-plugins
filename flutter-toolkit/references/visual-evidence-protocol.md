...
1. 해당 항목에 `[미검증]` 을 달고 네 칸을 채운다 — **막는 것**(실행한 명령과 그 실패 출력) · **시도한 우회**(위 세 확인과 채널을 확인한 순서. 정말 없으면 `없음 — 이유`) · **통제 불가 사유**(한 문장) · **재검증 명령**(채널이 생기면 돌릴 명령). 칸의 뜻은 `harness/docs/guides/skill-design-guide.md` §3.7 5 조항 3 항이 정한다
2. 네 칸 가운데 하나라도 비면 평가 측이 `[미검증:INVALID]` 로 센다 — 사유 한 줄로 끝내지 않는다
...
- 기준 캡처: <본 것을 요소 단위로 서술 | 신규 | [미검증] — 네 칸은 아래 미검증 줄>
- 기준 캡처 파일: <경로 | 신규 | [미검증] — 네 칸은 아래 미검증 줄>
- 재캡처: <본 것을 요소 단위로 서술 | [미검증] — 네 칸은 아래 미검증 줄>
- 재캡처 파일: <경로 | [미검증] — 네 칸은 아래 미검증 줄>
...
Good: 채널 없음 → "[미검증] 막는 것: 채널 감지 출력(golden 0 · integration_test 없음 · .mcp.json 에 캡처 서버 없음) · 시도한 우회: 세 채널을 순서대로 확인 · 통제 불가 사유: 이 환경에 캡처 채널이 없다 · 재검증 명령: 채널을 붙인 뒤 같은 캡처" → 부분 완료로 보고 + 확인 요청 2건 제시
```
