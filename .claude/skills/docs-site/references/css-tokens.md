...
| **Bambu Kit** | `#14B8A6` | `#5EEAD4` | `rgba(20,184,166,0.12)` | `rgba(20,184,166,0.06)` |
| **Onboarding Kit** | `#7C8AFF` | `#9AA5FF` | `rgba(91,108,255,0.12)` | `rgba(91,108,255,0.06)` |
...
accent 는 **텍스트 색으로도 쓰이므로** hue 만으로 고를 수 없다. `--surface2` 위에서
WCAG AA(4.5:1)를 넘어야 한다. `--bg` 위만 보면 통과인데 카드 안에서 미달인 경우가 나온다.

- **Onboarding Kit** 은 `#5B6CFF` → `#7C8AFF` 로 올렸다. 원래 값은 `--surface`(#181825) 위에서
  4.21 이었다. hue 는 유지된다.
