...
6. **문서에 적은 대비 수치가 실제와 맞는지**

   ```bash
   python3 scripts/check-contrast-claims.py
   ```

   위 5 번은 **렌더된 픽셀**을, 이건 **문서가 사실이라고 적어 놓은 수치**를 본다.
   페이지가 접근성을 지키면서도 틀린 설명을 달고 있을 수 있고 5 번은 그걸 못 잡는다
   (실측 2026-09-05: `typography-scale.html` 이 4.61:1 을 "3.5:1 FAIL" 로 적고 있었다 —
   디자인 가이드가 기준을 틀리게 가르치던 셈이다).

하나라도 실패하면 수정 후 재검증한다.

**수치가 틀렸을 때 색을 바꾸지 마라.** 적힌 수치가 틀린 것이지 색이 틀린 게 아니다.
색을 바꾸면 그 자리의 시각적 예시가 달라져 버린다 — 수치와 판정 라벨만 실제 값으로 맞춘다.
