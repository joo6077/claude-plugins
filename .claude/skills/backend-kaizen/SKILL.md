...
5. **validate-plugin.py 실행 없이 완료 선언 금지** — 카이젠 세션 종료 시 반드시 `scripts/validate-plugin.py backend-kit`을 실행하여 **등록된 검사 전부** 상태를 확인하라. 회귀가 발생하면 즉시 수정한다.
...
카이젠 세션 시작/종료 시 `scripts/validate-plugin.py backend-kit` 을 실행하여 등록된 검사 전부의 상태를 확인하고 결과를 개선 우선순위에 반영한다.
