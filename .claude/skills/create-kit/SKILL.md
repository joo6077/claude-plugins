...
2. **기존 킷 패턴 3종을 벗어나지 마라** — 원칙 기반 킷(backend-kit, design-kit, infra-kit)은 동일한 3스킬 패턴을 따른다: guide(가벼운 리뷰) + audit(체계적 감사) + system/init(초기 세팅). 4번째 스킬이 필요하면 기존 3개에 흡수할 수 없는지 먼저 검토하라. **예외: 언어 전용 워크플로우 킷(flutter-toolkit, rust-kit 등)은 다종 스킬 패턴을 허용한다** — 스캐폴딩, 코드 생성, 빌드/테스트 실행 등 워크플로우 스킬이 3종 이상 필요하기 때문이다.
