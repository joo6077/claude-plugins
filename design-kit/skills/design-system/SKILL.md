...
8. **DTCG 이름 규칙** — 토큰·그룹 이름에는 `{` `}` `.` 를 쓰지 않고, 이름을 `$` 로 시작하지 않는다 (`$` 접두는 `$value` · `$type` 같은 속성 키 몫이다). 계층은 JSON 그룹을 중첩해서 만든다. `.` 는 참조를 적을 때 그룹 이름을 잇는 자리(`{color.text.primary}`)에만 나오며 이름 안의 문자가 아니다. 출처: [DTCG Format Module 2025.10](https://www.w3.org/community/reports/design-tokens/CG-FINAL-format-20251028/).
...
- **Figma Variables → Tokens Studio → Style Dictionary v5 파이프라인 안내** — 토큰 명세 생성 시, Figma Variables(primitive + semantic) → Tokens Studio DTCG JSON 내보내기 → Style Dictionary v5 플랫폼별 변환 → Git 동기화 파이프라인을 사용자에게 안내한다. Code Syntax 활성화, Description 필드 활용, Scope 제한을 권장한다. v5 는 Node.js 22 이상이 필요하고, 토큰이 아닌 값을 가리키는 참조 · `.value` 를 붙인 참조 · 참조 구문 바꾸기를 더는 받지 않는다 — v4 설정을 옮길 때 이 셋을 먼저 본다. `convertTokenData` 객체 출력의 프로토타입 오염 취약점을 v5.5.5(2026-09-20)가 고쳤으니 그 아래 판은 쓰지 않는다. 출처: research-log §I, [Style Dictionary v5.0.0](https://github.com/style-dictionary/style-dictionary/releases/tag/v5.0.0), [v5.5.5](https://github.com/style-dictionary/style-dictionary/releases/tag/v5.5.5), [v5 Migration](https://styledictionary.com/versions/v5/migration/).
