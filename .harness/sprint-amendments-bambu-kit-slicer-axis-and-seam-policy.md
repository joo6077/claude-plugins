...
### 이번 실행에서 새로 드러난 것 (3 건 전부 조치 완료 · 2026-09-14)
...
| # | 조치 | 근거 |
| --- | --- | --- |
| 1 | 키 종류 색인을 제조사 전체로 넓혔다. 부모 체인 해석은 제조사 안에 그대로 뒀다 | 도그푸드 산출물의 `[미검증]` 8 건 → **0 건**. 음성 대조는 안 깨졌다 (실패 1 건 유지 · 검사 제거 시 통과) |
| 2 | `seam-recipes.md` §2.2 에 «둘레 20 mm 미만에서는 상한과 하한이 양립하지 않는다» 절 신설 + 파생 페이지 반영 | `0.15 x 둘레 >= 3` → 둘레 20 mm 가 경계. 둘레별 표와 «가장 작은 루프를 적어라» 규칙 포함 |
| 3 | `scripts/check-installed-sync.py` 경고에 «Skill 도구를 쓰지 마라 — 설치본을 부른다» 두 줄 추가 | 아래 |

3 번은 규칙 문서를 늘리는 대신 **이미 뜨는 훅 경고에 붙였다.** 이 어긋남이 실제로 문제가 되는
자리가 "고친 스킬을 돌려 확인할 때" 하나뿐이고, 그 시점에 경고가 이미 사용자 눈앞에 뜨기 때문이다.

경고 경로가 살아 있는지 **실제로 어긋나게 만들어** 확인했다. 첫 시도는 `bambu-kit/README.md` 를
고쳤는데 경고가 안 떴다 — 이 검사는 `skills/` · `agents/` 만 보므로 **내 시험이 틀린 것**이었다.
대상 안의 `SKILL.md` 로 다시 하니 떴다.

```text
[킷 동기화] 레포 내용이 설치본보다 앞선 킷이 있습니다 — 버전이 같아서 갱신이 걸리지 않습니다.
  - bambu-kit v0.9.1
  해소: bash scripts/release.sh bambu-kit patch  → PR 머지 → claude plugin update bambu-kit@joo6077-plugins
  ⚠ 고친 스킬을 돌려 확인해야 하면 Skill 도구를 쓰지 마라 — 설치본을 부른다.
     위 순서로 먼저 배포하거나, 레포 bambu-kit/skills/<이름>/SKILL.md 를 기준 삼아 직접 실행하라.
```

되돌린 뒤에는 다시 출력 0 건이다. **어긋남이 없을 때 조용한 것과 검사가 죽어 있는 것을 구분**하려면
이렇게 양쪽을 다 봐야 한다.
