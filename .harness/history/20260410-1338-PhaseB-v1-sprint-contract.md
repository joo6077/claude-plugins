...
## Architecture
- [ ] AR-01: `docs/process/kaizen-flow.html`의 subtitle(166행 부근), `<h2>` 제목(230행 부근), 본문 설명, 시뮬레이션 안내문(460행 부근)의 "6-Phase" 표기가 모두 "9-Phase"로 일관 갱신되고, 트리거 테이블의 "Full Phase 1→2→3→4→5→6→Final" 문자열도 Phase 9까지 포함되도록 수정된다
- [ ] AR-02: kaizen-flow.html `.phase-grid`에 backend/infra/rust 각 Phase `.phase-card`가 design-kit(`#phase-6`) 다음·`#phase-final` 이전에 삽입되고, 카드 id는 `phase-7`, `phase-8`, `phase-9`로 지정되며, 카드 간 연결선(`conn-*`) 인덱스가 Phase 순서와 일치한다
- [ ] AR-03: `<style>`에 `.phase-badge.p7`, `.phase-badge.p8`, `.phase-badge.p9` CSS 클래스가 추가되고 각 신규 카드에서 사용된다 (기존 색상 팔레트 재사용)
- [ ] AR-04: "개별 Phase 실행" 코드 블록(476행 부근)에 `/kaizen-orchestrator phase7|phase8|phase9` 라인이 각각 추가되고, 상단 트리거 테이블(177~181행)에 backend/infra/rust 수동 실행 행이 추가된다
- [ ] AR-05: 시뮬레이션 스크립트(`startSim`)의 Phase 6 블록 다음에 Phase 7/8/9의 `activate → log → complete → activateConn` 시퀀스가 연속된 T+m 시간축으로 추가되고, "✅ 카이젠 사이클 완료" 로그가 Phase 9 이후·Final 이전에만 발생하지 않도록 순서가 조정된다 (Final 이후에 출력)

## Anti-patterns
- [ ] AP-01: 버전 문자열을 하드코딩하지 않는다 — 모든 버전 참조는 해당 plugin.json 또는 marketplace.json에서 읽는 지침으로 기술한다
- [ ] AP-02: `git push --force` 지침이 추가되지 않는다 — SKILL.md Step 8의 push 명령은 일반 push만 허용한다

## Reusability
- [ ] RE-01: 다른 곳에서도 사용 가능한 컴포넌트를 private으로 만들지 않았다
- [ ] RE-02: 프로젝트에 이미 동일/유사 컴포넌트가 있으면 새로 만들지 않고 재사용했다 — HTML 카드는 기존 `.phase-card` 클래스 재사용, 신규 CSS는 배지 색상만 추가
...
- [ ] DG-03: `python3 scripts/sync-docs.py --check-only` 실행 시 에러 0개 (README 동기화 영향 없음 확인)
