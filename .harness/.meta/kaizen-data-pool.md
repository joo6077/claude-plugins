...
- [2026-04-10] **claude-plugins**: AP-03: react-transition-group이 SKILL.md Gotcha 1 금지 목록 및 agent.md 절대 금지 라이브러리 목록에 미포함
- [2026-04-10] **claude-plugins**: AP-01: react-form Gotchas에 Zustand/TanStack Query 상태 분리 원칙 미명시
...
- [2026-04-10] **claude-plugins**: SK-06 경계 판정 명확화: 컨셉 단계 hex 기재 허용 범위를 계약에 더 구체적으로 기술할 것 (예: '무드보드 CSS 변수는 방향 시각화용으로 허용')
...
- sprint-feedback.md: 119 lines
- history sprint-contracts: 14
- 최근 contracts:
  - 20260411-1318-sprint-contract.md
...
```markdown
# Sprint Feedback
Feature: AdmHtmlEditorWidget 실구현 (html_editor_enhanced + 순수 텍스트 서식 툴바)
...
- [x] UI-01: dev_widget_admin_screen line 417 렌더링에 실제 HtmlEditor 위젯 표시 — PASS [L2]
  - 근거: `dev_widget_admin_screen.dart:417` `const Text('--- 팝업 공지 html 편집기(AdmHtmlEditorWidget) ---')`, `:418` `const AdmHtmlEditorWidget(AdmHtmlEditorWidgetProps(hint: '팝업 공지 내용을 입력하세요', height: 400))` — 빈 Container가 아닌 실제 위젯 사용 확인.

- [x] UI-02: 툴바에서 InsertButtons 계열 7종 완전 제거 — PASS [L3]
  - 근거: `html_editor_widget.dart:96-104` `defaultToolbarButtons: [StyleButtons(), FontSettingButtons(), FontButtons(), ColorButtons(), ListButtons(), ParagraphButtons(), OtherButtons()]` — InsertButtons 미포함. `const HtmlToolbarOptions(...)` 명시적 오버라이드로 라이브러리 기본값 덮어씀.

- [x] UI-03: 텍스트 효과 관련 버튼 모두 노출 (개정된 계약 기준) — PASS [L3]
  - 근거: Sprint Contract 개정본: "StyleButtons, FontSettingButtons, FontButtons, ColorButtons, ListButtons, ParagraphButtons, OtherButtons 전체 기본값(모든 필드 true) 포함". `html_editor_widget.dart:97-103` 7개 그룹 모두 기본 생성자(모든 필드 기본값 true)로 포함. InsertButtons만 리스트에서 완전 제외. 계약 개정본과 코드 일치.
```
...
- `.harness/history/20260410-2057-react-kit-phase7-sprint-contract.md`
- `.harness/history/20260410-2122-react-kit-phase8-sprint-contract.md`
- `.harness/history/20260410-2256-react-kit-phase10-final-sprint-contract.md`
- `.harness/history/20260410-PhaseA-prev-sprint-contract.md`
- `.harness/history/20260411-1315-plugin-validation-infra-sprint-contract.md`
