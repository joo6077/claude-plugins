...
11. **IaC 시크릿은 Ephemeral 또는 참조만 (Phase 8 리서치)** — Terraform/OpenTofu 세팅 시 시크릿을 state 에 평문 저장하지 말고 `ephemeral` 블록(Terraform 1.10+) 또는 write-only 인수, OpenTofu native state encryption 을 권장 규격에 포함한다. `sensitive` 마킹만으로는 state 평문 저장이 유지된다. 출처: [Terraform ephemeral](https://developer.hashicorp.com/terraform/language/ephemeral) · [OpenTofu state encryption](https://opentofu.org/docs/v1.11/language/state/encryption/).
