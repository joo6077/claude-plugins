...
> 아래 사본은 그 정본의 복제본이며, 본 에이전트는 임계값이나 마커 의미를 여기서 다시 정의하지 않는다.
...
재검증 명령 예: `brew install kubeconform && kubeconform -strict k8s/`

### 인프라 도메인 적용 노트 (정본 재정의 아님 — 사본의 「`[미검증]` 은 검증 도구·환경 부재 전용이며」 조항의 4 분기를 이 도메인에 매핑한 것)
