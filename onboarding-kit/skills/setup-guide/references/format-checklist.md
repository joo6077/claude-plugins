...
- ⚠️ 주의 박스 — 흔한 실수
- ❌ Deprecated / ✅ 현재 권장 박스 — **1차 출처가 실제로 deprecated 라고 말한 경우에만.** 이 박스를 쓰면 그 Step 의 `**출처:**` 줄에 "출처가 deprecated 라고 명시" 라는 근거를 함께 적는다. 한국어 1 차 출처면 그 표기 그대로 적는다 — "출처가 '지원 종료' 라고 명시", "폐지", "중단". G4 는 영문 deprecated/sunset/removed 와 한국어 지원 종료/폐지/중단/서비스 종료/단종 을 모두 근거로 인정한다. 근거 없는 박스는 `guide_gate` G4 가 잡는다. "권장하지 않음" · "새 방식이 권장됨" 을 deprecation 으로 승격하지 마라
