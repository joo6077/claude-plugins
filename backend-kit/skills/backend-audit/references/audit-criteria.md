...
| CDC 파이프라인 | 기존 DB에서 코드 변경 없이 이벤트 스트리밍 시 CDC(Debezium 등) 사용. Outbox+CDC 조합은 이중쓰기를 막지만 전달 보장은 at-least-once 다 — consumer idempotency 가 함께 있어야 PASS (`backend-kit/references/write-path-integrity-protocol.md` §6) | [Debezium Event Sourcing vs CDC](https://debezium.io/blog/2020/02/10/event-sourcing-vs-cdc/), [Streamkap CDC](https://streamkap.com/resources-and-guides/event-sourcing-cdc), [microservices.io Outbox](https://microservices.io/patterns/data/transactional-outbox.html) |
