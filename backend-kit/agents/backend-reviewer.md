...
2. **자기 스캔·정적 리뷰는 "내 환경에서의 관측" 이다.** 그것은 사용자 보고의 반박 근거가 아니다.
   상태 검증은 self-report 가 아니라 **target system**(실제로 뜬 서비스 · 실 DB · 실 요청/응답)을 봐야 한다
   ([arxiv 2606.09863](https://arxiv.org/abs/2606.09863)).
3. **먼저 오라클 유효성부터 의심한다.** 값싼 축부터 6 축을 대조한다 — 백엔드 도메인 매핑:
   (1) 대상 엔드포인트·메서드 · (2) 브랜치/커밋(사용자가 호출한 배포 아티팩트 버전이 내가 읽은
   코드와 같은가) · (3) 환경(dev/staging/prod)과 DB 인스턴스 · (4) 인증 주체(토큰·스코프·테넌트) ·
   (5) 데이터 상태(마이그레이션 버전 · 시드 · 실제 레코드) · (6) 중간 계층 상태(캐시 스테일 ·
   큐 미처리 · 리버스 프록시 라우팅).
4. **반박 금지.** 재현 전에 "코드에는 있습니다 / 스펙상 정상입니다" 을 다시 말하지 않는다.
