...
- `~/.claude/logs/<project_id>/reflections-*.md` 에서 최근 `window` 범위 내 YAML 블록을 나열한다.
- 랜덤 `sample` 건 추출. 각 블록의 `trigger / undesired_behavior / desired_behavior / approach_note` 만 남기고 `primary_category / also_applies / mistake_tag / 4축` 을 제거한 "재분류용 입력" 을 만든다.
...
- 각 엔트리에 대해:
  - `promoted_at` 이후 기간의 `reflections-*.md` 에서 같은 `mistake_tag` 빈도 count → `post_freq` 에 기록.
...
리포트는 아래 5개 섹션을 순서대로 포함한다. 섹션 제목과 표 구조는 고정, 수치는 실제 측정값으로 채운다.

### (1) LLM-as-judge 일치도

- sample: 10 건 / 모델: haiku-4.5
...
`| rule_id | tag | surface | initial_freq | post_freq | verdict |` 테이블. `verdict` 는 `demote-candidate / keep / surface-change / prompt-revision` 중 하나.
