...
description: >
  ...
  현행 모델(gpt-6-astra / gpt-5.6-sol) 기준 최신 prompting 가이드를 리서치하여 템플릿 개선 diff를 제안한다.
...
- **모델을 명시해서 부른다.** `~/.codex/config.toml` 기본값이 `gpt-5.6-luna`(가장 작은 등급, 단순 분류용)라 그대로 두면 리서치를 가장 작은 모델이 받는다. 리서치는 `--model gpt-5.6-sol` 이상을 붙인다. 모델 미지정은 자동 전환 버그에도 노출된다(openai/codex #45009). 호출 실패 시 동일 프롬프트 1회 재시도 → 그래도 안 되면 WebSearch fallback으로 전환하되 출처 미검증임을 명시한다.
...
- **과한 절차 지시 금지.** 현행 GPT-5 계열은 과한 `ALWAYS`/`NEVER`/단계 나열에서 overthinking·latency가 늘고, **모순된 규칙이 빠진 세부보다 더 큰 불안정을 만든다**. outcome·constraints·done/verification만 선명하게(근거: 번들 prompting-guide — 가벼운 프롬프트가 평가 점수 10~15%↑, 토큰 41~66%↓).
...
4. 리뷰 시점 독립 평가로 아래 약점 패턴을 집계한다(위임 시점 self-score 아님):
   ...
   - **Fallback 빈도**: WebSearch fallback / "출처 미검증" 표기 / 404·400 실패가 반복되는가.
   - **멈춤 빈도**: `stalled` 엔트리의 비율과 끊긴 지점. 단 이것은 **프롬프트 결함이 아니라 상류 회귀**일 수 있으므로(codex 0.153.4~, openai/codex #44842) cli 버전·호출 경로별로 갈라 본 뒤에만 프롬프트 탓으로 돌린다.
   - **조회 품질**: `Search activity` 에 남은 주소가 1차 출처인가, 검색 결과 페이지만 맴돌았는가.
...
2. **Codex (read-only)** — 웹 리서치 위임. `~/.claude/bin/codex-research <프롬프트파일> <출력파일>` 로 부른다(상한·재시도 내장). 직접 부를 때는 `</dev/null` 을 반드시 붙인다 — stdin 이 열려 있으면 codex 가 무한 대기한다. companion 경유는 미완료율이 네 배 이상이다. 대상: OpenAI 공식 prompt-guidance, latest-model 가이드, output-contract/grounding 패턴.
3. codex 호출 실패 시 1회 재시도 → WebSearch fallback(출처 미검증 명시).
...
- 리서치 위임을 모델 지정 없이(기본 `gpt-5.6-luna`로) 부르지 마라.
- 멈춤을 프롬프트 결함으로 단정하지 마라 — cli 버전·호출 경로별로 갈라 보고 나서 판단한다.
