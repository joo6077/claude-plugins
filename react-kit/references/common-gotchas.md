...
금지 목록: `motion` / `framer-motion` / `dnd-kit` / `react-spring` / `react-transition-group` / `react-dnd` / `react-beautiful-dnd` / `@formkit/auto-animate` / `gsap` / `lottie-react` / `animate.css`
