...
11. **`@hookform/resolvers` v5.2.2 Standard Schema 지원** — Standard Schema 기반 resolver 가 추가됐다. 단, Zod v4 타입 호환성 이슈는 여전하므로 `import { z } from 'zod/v3'` workaround 를 유지한다. Standard Schema 로 전환 시 resolver import 경로가 변경되므로 점진적 마이그레이션을 권장한다.
