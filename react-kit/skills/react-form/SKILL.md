...
10. **RHF v8 beta — 아직 프로덕션 사용 금지** — React Hook Form v8.0.0-beta.1 (2026-01-11) 이 공개됐으나 breaking changes 포함. **v7 라인을 사용**한다 (2026-09-24 조회 npm `latest` 7.88.0). v8 stable 릴리스 전까지 마이그레이션하지 않는다.
11. **`@hookform/resolvers` Standard Schema 지원 + Zod 4 호환은 v5.1+ 에서 해소 (2026-08-13 정정)** — Standard Schema 기반 resolver 는 v5.2.2 에서 추가됐다. **Zod v4 타입 호환성 이슈는 `@hookform/resolvers@5.1.0` 에서 이미 해소됐다** (2026-09-24 조회 npm `latest` 5.9.1). `import { z } from 'zod/v3'` workaround 를 기본 전제로 유지하지 마라 — 그 alias 는 **legacy resolver(v5.1.0 미만)** 에 묶인 프로젝트 전용이다. Standard Schema 로 전환 시 resolver import 경로가 변경되므로 점진적 마이그레이션을 권장한다.
