...
게이트는 옵션 목록(§11.1)의 프리셋 종류 줄과 파일 `type` 을 대조한다 — 손으로 유지하는 목록이 아니다.
2026-09-15 전에는 프로파일에 키가 등장하는 종류로 판정했는데, 제조사 프로파일의 실수가 허용 근거가 됐다
(뱀부 process 기본 프로파일 `fdm_process_common.json` 이 filament 키 `pre_start_fan_time` 을 담고 있다).
...
### 11.1 키가 어느 슬라이서에 있는가 — 옵션 목록이 1 순위다 (2026-09-15 갱신)

**1 순위 근거는 `references/option-keys/` 의 슬라이서 버전별 목록이다** (`orca-2.4.2.tsv` ·
`bambu-02.08.02.61.tsv`). 두 슬라이서의 태그 소스에서 옵션 등록부 · 옛 이름 처리 함수 · 프리셋 기본값을
부분 빌드한 판정 프로그램(`bambu-kit/scripts/option-key-probe/`)이 **슬라이서 자신의 함수로** 뽑았다.
Phase 4.3 게이트가 이 목록으로 키 존재 · 프리셋 종류 · enum 값을 판정한다. 설치본 버전이 바뀌면
`bash bambu-kit/scripts/option-key-probe/build-option-list.sh <orca|bambu>` 로 새 목록을 만든다 —
목록이 없는 버전은 게이트가 `[미검증]` 으로 보고하고, 다른 버전 목록으로 대신 검사하지 않는다.

| 줄 종류 | 뜻 | 오르카 2.4.2 | 뱀부 02.08.02.61 |
| --- | --- | --- | --- |
| `canonical` | JSON 을 불러올 때 그대로 받아들여지는 키 | 749 | 679 |
| `renamed` | 불러올 때 새 이름으로 바뀌는 옛 키 | 42 | 23 |
| `process` · `filament` · `machine` | 그 종류 프리셋이 받는 키. 설정 가져오기는 여기 없는 키를 지운다 | 356 · 127 · 161 | 277 · 143 · 136 |
| `enum` | enum 키가 받는 값 (`nil` 은 소재별 덮어쓰기의 "값 없음") | 384 | 385 |
| `renamed-value` | 불러올 때 새 값으로 바뀌는 옛 값 (예: 오르카 `top_surface_pattern` `zig-zag` → `rectilinear`) | 32 | 19 |
| `header` | 옵션이 아니라 불러오기가 따로 읽는 파일 머리 키 (`name` · `version` · `filament_id` · `url` …) | 36 | 42 |

목록 기준으로 받는 키는 오르카에만 229 · 뱀부에만 159 · 양쪽 520 이다. 목록이 맞는지는 세 가지로
확인했다 (2026-09-15): 설치본 명령줄 자르기에 가짜 키 · 슬라이서 전용 키 · 옛 키 · 옛 값 · 별칭 · 무시
목록 · 받지 않는 enum 값 14 사례를 넣은 결과와 전부 일치, 자르기 결과 3mf 의 설정 키가 전부 목록 안,
제조사 프로파일 전체(오르카 11,554 · 뱀부 3,390)에서 받지 않는 값 0 건.

**목록이 못 담는 것 — 불러오기 함수 안의 특수 처리.** 목록은 옛 이름 처리 함수(`handle_legacy`)만 따라간다.
`load_from_json` 안에 따로 적힌 처리 3 건은 목록에 없다: 뱀부 `reduce_infill_retraction: "0"` →
`reduce_infill_retraction_mode: Disabled` (키 자체는 버린다), `support_type: hybrid(auto)` → `support_style` 을
`tree_hybrid` 로, `wall_infill_order` 의 채움 우선 값 → `is_infill_first`. 게이트는 첫째를 `모르는 키` 로 막고
나머지 둘은 `옛 값` 으로 알린다 — 새 키 이름으로 고쳐 쓰면 셋 다 해소된다.

**실행 파일 문자열은 2 순위 참고로만 쓴다.** 한계가 넷이다.

- **옛 이름과 무시 목록 키도 문자열로 들어 있다.** 옛 이름 처리 함수가 그 이름을 문자열로 비교하기 때문이다.
  목록의 옛 이름이 오르카 42 개 · 뱀부 23 개 전부 실행 파일 문자열에 그대로 있다. 뱀부
  `reduce_infill_retraction` 도 문자열에는 있지만 뱀부는 그 키를 받지 않는다 (위 특수 처리).
- **기본 최소 길이 4 글자에 걸린다.** 3 글자 키 `url` 은 `strings -a` 로 안 나온다 — 문자열로 볼 때는
  반드시 `strings -a -n 3` 으로 뽑는다.
- **명령이 실패해도 빈 출력이 나온다.** Xcode 사용권 동의 전에는 `strings` 가 오류로 끝나는데 출력이 비어
  "0 건" 으로 읽힌다 (실측 2026-09-14).
- **프리셋 종류와 enum 허용값을 말해주지 않는다.** 키 이름이 있다는 것만 안다.
...
| 근거 | 재는 법 | 무엇을 말해주나 |
| --- | --- | --- |
| 옵션 목록 | `references/option-keys/<슬라이서>-<버전>.tsv` 의 `canonical` · 종류 · `enum` 줄 | **그 슬라이서가 그 키·값을 받는가, 어느 프리셋에서 받는가** (1 순위) |
| 앱 실행 파일 문자열 | `strings -a -n 3 <앱>/Contents/MacOS/<앱>` 에 키 이름이 한 줄로 있는가 | 이름이 코드 어딘가에 있는가 — 옛 이름도 걸린다 (2 순위) |
| 프로파일 집계 | `Contents/Resources/profiles/**.json` 에서 키 등장 수 | 기본값과 다르게 쓰는 자리가 있는가 |

#### 실측 — 같은 키 집합을 두 방법으로 판정한 결과 (2026-09-14 · 목록 도입 전)
...
으로 오판하게 된다 — v4 초판이 실제로 그렇게 적었다가 2026-09-14 감사에서 걸렸다. 2026-09-15 옵션 목록으로
다시 재니 `reduce_infill_retraction` 은 문자열만 있고 뱀부가 받지 않는 키였다 — 나머지 6 개는 양쪽 목록에 있다.
...
| `reduce_infill_retraction` | 문자열만 있음 | 0 건 | 733 건 | **안 된다** — 뱀부 목록에 없다 (§11.1 특수 처리). 뱀부는 `reduce_infill_retraction_mode` 를 쓴다 |
...
| `reduce_infill_retraction_mode` | process | 없음 | 0 건 | 오르카는 `reduce_infill_retraction` (모드 없는 불리언) — 그 키를 뱀부는 받지 않는다 (§11.1 특수 처리) |
...
1. **종류를 목록으로 확인한다.** 키 이름이 양쪽에 다 있어도 받는 프리셋 종류가 다를 수 있다. Phase 4.3
   게이트가 옵션 목록의 `process` · `filament` · `machine` 줄로 대조한다 (`retraction_minimum_travel` 은
   양쪽 모두 machine 에서만 받는다).
2. **enum 값을 목록으로 확인한다.** 같은 키라도 받는 값이 다르고, 벗어난 값은 오류 없이 기본값으로
   바뀐다. 실측: `seam_slope_type` 에 `hole` 을 넣으면 exit 0 · 경고 0 으로 경사가 사라지고, 뱀부
   `seam_position: aligned_back` 은 `aligned` 로 바뀐다. 게이트가 모든 enum 키를 `받지 않는 값` 으로 잡는다.
3. **음성 대조로 확인한다.** 목록이 의심되면 가짜 키를 하나 넣어 명령줄로 슬라이스한 뒤 결과 3mf 의
   `Metadata/project_settings.config` 에서 소멸하는지 본다. 소멸하면 "그 슬라이서가 모르는 키" 다.
