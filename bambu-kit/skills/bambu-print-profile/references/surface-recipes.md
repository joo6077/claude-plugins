...
### 2.8. 벽 예산 — 피처 폭이 벽을 못 담으면 갭필로 샌다 (2026-09-14 신설)

구멍 · 빈 칸이 있는 부품은 형상 클래스(§2.7)와 **무관하게** 한 가지를 더 계산한다. **벽이 실제로 들어가는지**다.
처음에는 `thin` 에서만 계산했는데, 2026-09-19 전부 `planar` 로 판정된 부품에서 같은 결함이 났다 (아래 실측).

### 계산식

```text
벽 예산 = outer_wall_line_width x 2 + inner_wall_line_width x 2 x (wall_loops - 1)
```

`0.12mm High Quality @BBL H2S` 실효값(외 0.42 · 내 0.45 · `wall_loops` 2) 기준 **1.74 mm** 다.

피처의 **국소 최소폭**이 이 값보다 작으면 벽 두 겹이 안 들어간다. 중앙값이나 수력직경으로 재면
안 된다 — 결함은 가장 좁은 자리에서 난다. 회전 캘리퍼로 루프별 최소 두께를 재라.

### 측정

`SKILL.md` Phase 1.0 형상 측정을 `WALL_LOOPS=<벽 수> OUTER_LINE_WIDTH=<외벽 선폭> INNER_LINE_WIDTH=<내벽 선폭>` 과 함께
돌린다. 부품마다 루프 위 점에서 **다른 루프**까지의 거리를 재서, 벽 예산보다 좁은 둘레 비율을 바닥 0.1 mm · 25 · 50 · 75 %
높이에서 낸다. 마지막 줄의 `_wall_budget_short_share`(전 부품 최댓값)를 process JSON 에 기록하면 Phase 4.3 게이트가
`classic` 과 함께 10 % 이상일 때 FAIL 한다. 10 % 는 추정이다 — 결함이 난 실측은 22~90 %, 결함이 없던 부품은 0 % 였다.

비율은 둘레 **길이**로 센다. 점 수로 세면 메시가 잘게 쪼개진 곡선 구간이 부풀어, 같은 뒤쪽 브래킷이 72.0 % 대신 58.8 % 로
나온다 (2026-09-19 두 방식 대조). 같은 루프 안에서 윤곽이 접혀 들어온 좁은 목은 재지 않는다.

**외벽 파편화 배수 `_wall_outer_block_ratio` 도 같이 잰다 (2026-09-26 신규).** 벽 예산이 부족해도 `classic` 이 외벽을
조각내지 않는 형상이 있고, 그때는 `classic` 이 더 낫다. 같은 3mf 를 **두 생성기로 각각 잘라** G-code 의
`; FEATURE:Outer wall` 블록 수를 세고 `classic` 쪽을 `arachne` 쪽으로 나눈다.

```bash
# 사본 3mf 의 Metadata/project_settings.config 에서 wall_generator 만 그 값으로 바꿔 자른다.
# 나머지 값은 건드리지 않는다 — 한 축만 바꿔야 배수가 그 축의 효과가 된다.
for V in classic arachne; do
  /Applications/BambuStudio.app/Contents/MacOS/BambuStudio --slice 1 --outputdir "out-$V" "copy-$V.3mf"
  printf '%s %s\n' "$V" "$(grep -c '^; FEATURE:Outer wall' "out-$V/plate_1.gcode")"
done
# _wall_outer_block_ratio = classic 블록 수 나누기 arachne 블록 수 → process JSON 에 소수 3 자리로 기록
```

1.3 이상이면 `arachne` 로, 그 미만이면 `classic` 을 그대로 둔다. **1.3 은 추정이다** — 조각난 실측이 1.850 (래티스 통),
안 조각난 실측이 0.993 (H2 배기관) 이다. Phase 4.3 게이트는 이 값이 없으면 `classic` 을 통과시키지 않는다 — 안 재고
넘어가는 길을 두지 않기 위해서다.
...
| --- | --- | --- |
| >= 벽 예산 | 정상 | 그대로 |
| < 벽 예산, >= 외벽 2겹(0.84 mm) | **벽이 못 들어감** | `_wall_outer_block_ratio` 를 재라 — 1.3 이상이면 `wall_generator` 를 `arachne` 로, 그 미만이면 `classic` 유지 |
| < 외벽 2겹 | 벽 1 겹도 불가 | 모델 쪽 문제. 사용자에게 보고 |

`classic` 생성기는 벽이 못 들어가는 자리를 **갭필로 때운다.** 갭필 선폭은 그 자리 형상이 정하므로
자리마다 달라지고, 결함이 **매 출력 같은 위치에 재현**된다. `arachne` 는 벽 선폭을 형상에 맞춰
바꿔 갭필을 만들지 않는다.

**다만 갭필이 있다는 것 자체가 겉에 드러나는 것은 아니다.** 갭필이 외벽과 안쪽 벽 **사이**에 들어가면
외벽이 이미 겉면을 그린 뒤라 안쪽에 숨는다. 겉에 드러나는 것은 `classic` 이 **외벽까지 조각낼 때**다 —
조각 하나하나가 압출 시작·종료이고 이음매 후보지가 늘어난다. 그래서 판정은 벽 예산 부족 비율만으로
하지 않고 위 「측정」 의 파편화 배수를 함께 본다. 반대로 `arachne` 는 벽 선폭을 가변으로 만들므로,
벽 두께가 연속적으로 변하는 곡면에서는 **안쪽 벽이 굵어져 외벽을 밀어** 겉면에 요철을 만들 수 있다.

### 실측 (2026-09-14, 래티스 통)

스트럿 국소 최소폭 1.15~1.51 mm — 벽 예산 1.74 mm 미만이라 **56~90% 구간에서 두 겹이 안 들어간다.**
같은 부품을 두 생성기로 잘라 비교했다.

| `wall_generator` | 갭필 구간 | 외벽 블록 | 갭필 선폭 범위 |
| --- | --- | --- | --- |
| `classic` | **16,123** | 18,267 (파편화) | 0.076~0.697 mm (9 배 요동) |
| `arachne` | **0** | 9,875 | — |

`classic` 은 외벽까지 두 배로 파편화한다 — 조각 하나하나가 압출 시작·종료라 이음매 후보지도 두 배다.

### 실측 (2026-09-19, H2 AMS Flipper — `planar`)

10 개 부품 전부 `planar` 였고 벽 4 겹(외 0.42 · 내 0.45, 예산 3.54 mm)으로 출력했다. 첫 출력 2~3 층에서 모서리 · 갈림길 ·
둥근 경첩 구멍 둘레에 덩어리가 솟아 사용자가 멈췄다.

| 부품 | 바닥 층 부족 비율 | 최소 살 |
| --- | --- | --- |
| 뒤쪽 브래킷 (구멍 21 개) | 71.9 % | 2.01 mm |
| 80° 브래킷 | 72.1 % | 2.53 mm |
| AMS2 브래킷 | 0 % | 2.29 mm |

같은 판을 두 생성기로 잘라 비교했다 (뱀부 스튜디오 02.08.02.61 명령줄).

| `wall_generator` | 전체 갭필 | 갭필 조각 | 갭필 선폭 | 1 층 되감기 |
| --- | --- | --- | --- | --- |
| `classic` | **253.0 m** | 165,952 | 0.069~0.748 mm | 353 회 |
| `arachne` | 0.4 m | 1,558 | 0.068~0.404 mm | 206 회 |

> 잰 방법 (2026-09-25 추가): 명령줄로 자른 G-code 에서 쟀고 호(`G2` · `G3`)를 넣었는지 기록이 없다 — 같은 수치를 다시 잴 때는
> `SKILL.md` Phase 4.4 「G-code 로 길이 재기」 블록으로 재고 호 비율을 같이 적는다.

둥근 경첩 구멍은 바깥 4 겹과 안쪽 4 겹 사이 틈이 원을 따라 한 바퀴 이어져 갭필 고리가 됐고, 사용자가 "원형으로 있는 게
엄청 심하다" 고 짚은 자리가 거기였다. `arachne` 로 바꾼 재출력에서 사용자가 "이번엔 괜찮네" 라고 확인했다.

### 실측 (2026-09-26, H2 배기관 — 곡면 꺾인 관)

**이 사례에서 `arachne` 가 오히려 표면을 망쳤다.** MakerWorld 2634303, 9 방향 꺾인 배기관, ABS, 벽 4 겹(예산 3.36 mm),
부족 비율 93.3 % — 지금까지 셋 중 가장 심한데도 `classic` 이 정답이었다.

사용자가 실물 출력 사진으로 "꺾이는 구간 안쪽과 바깥쪽 같은 높이가 거칠다" 고 보고했고, 보낸 G-code 설정 기록으로
`arachne` 판임을 확정한 뒤 두 생성기로 재슬라이스해 비교했다 (뱀부 스튜디오 02.08.02.61 명령줄, 호 포함 측정).

| `wall_generator` | 갭필 | 갭필 블록 | **외벽 블록** | 폭 0.44 mm 초과 안쪽벽 | 모델 출력 시간 |
| --- | --- | --- | --- | --- | --- |
| `classic` | 117.84 m | 1,640 | **1,202** | 3.01 m (0.2 %) | 4h 31m 28s |
| `arachne` | 0.02 m | 5 | **1,211** | **140.91 m (11.3 %)** | 4h 50m 19s |

`classic` 이 외벽을 조각내지 않았다 (1,202 대 1,211 — 배수 0.993). 갭필 1,640 블록의 바로 앞 피처는 `Outer wall` 790 ·
바로 뒤는 `Inner wall` 759 로, 갭필이 외벽과 안쪽 벽 **사이**에 들어가 겉에 드러나지 않는 자리였다. 반대로 `arachne` 는
안쪽 벽을 기준 0.45 mm 보다 굵게(95 % 지점 0.516 mm) 만들어 외벽을 밀었고, 그 분포가 사용자가 짚은 두 자리와 겹쳤다
(Z 40~49 mm 22.4 % · Z 50~59 mm 18.1 %, 매끈했던 직선 구간 Z 0~19 mm 는 2.6~5.3 %). 시간도 19 분 더 길었다.

### 세 사례를 한 척도로 (2026-09-26)

갈리는 축은 벽 예산 부족 비율이 아니라 **`classic` 이 외벽을 조각내는지**다. 부족 비율이 가장 높은 사례에서
`classic` 이 정답이었으므로, 비율만으로 판정하면 이 형상을 놓친다.

| 사례 | 형상 | 벽 예산 부족 | `classic` 외벽 블록 | `arachne` 외벽 블록 | 배수 | 맞는 선택 |
| --- | --- | --- | --- | --- | --- | --- |
| 래티스 통 (2026-09-14) | 스트럿 격자 | 56~90 % | 18,267 | 9,875 | **1.850** | `arachne` |
| H2 AMS Flipper (2026-09-19) | 평면 + 구멍 21 개 | 71.9~72.1 % | 미기록 | 미기록 | 미측정 | `arachne` |
| H2 배기관 (2026-09-26) | 곡면 꺾인 관 | **93.3 %** | 1,202 | 1,211 | **0.993** | **`classic`** |

격자·구멍 많은 평면은 벽이 못 들어가는 자리가 외벽 경로 **중간**에 나타나 외벽이 조각난다. 곡면 관은 벽 두께가
연속적으로 줄어 외벽이 온전한 고리로 남고 갭필이 그 안쪽에 들어간다. 2026-09-19 사례는 배수를 재지 않았으므로
같은 척도로 다시 재려면 그 판을 두 생성기로 잘라야 한다.
...
지지대 없이 허공에 거는 구간만 외벽보다 1.7 배 빠르다. 외벽을 안 낮춘 설정도 부모 `50` 을 그대로 물려받으므로 같은 자리가 남는다.

| 부품 | 허공 위 압출 | 그 층 수 | 속도 | 팬 |
| --- | --- | --- | --- | --- |
| 뒤쪽 브래킷 A / B | 7.7 m / 7.8 m | 10 / 11 | `50` mm/s | 100% |
| 자동 급지 받침 A / B | 7.2 m / 7.0 m | 12 | `50` mm/s | 100% |
| 출력 나사 A / B | 1.8 m | 1 | `50` mm/s | 100% |
| 80° 브래킷 A / B | 1.4 m | 2 | `50` mm/s | 100% |

> 잰 방법 (2026-09-25 추가): 명령줄로 자른 G-code 에서 쟀고 호(`G2` · `G3`)를 넣었는지 기록이 없다 — 같은 수치를 다시 잴 때는
> `SKILL.md` Phase 4.4 「G-code 로 길이 재기」 블록으로 재고 호 비율을 같이 적는다.

층마다 조금씩 나오는 허공 구간(가장 긴 연속 5 층, 층당 평균 428 mm)은 둥글게 말려 올라가는 아랫면이다. 사용자 보고는
"아랫면 휘어진 부분이 밑으로 쳐지고 까끌까끌하다" 였다. Phase 4.3 게이트가 `_geometry_class` 기록이나 외벽 하향을 보면
`bridge_speed > 30` 을 FAIL 로 잡으며, 압출기 슬롯을 전부 읽어 슬롯 하나만 높은 경우도 잡는다 (2026-09-23 확대).
...
| **ABS / ASA** | 기본 `no ironing`, 실물 비교 뒤에만 `topmost` | `15-20` | `8-12%` | `0.12-0.18` | `0.3-0.5` | **기본 끔** — 후가공(vapor smoothing)이 가능하면 더더욱 |
| **TPU** | `no ironing` | — | — | — | — | **불가** — TPU 유연성으로 표면 drag |

**ABS 를 기본 끔으로 돌린 근거 (2026-09-22, H2 AMS Flipper)** — 판에서 가장 낮은 부품(출력 나사, 높이 9.08 mm)은 윗면이
한 층뿐인데 그 층 윗면 길이 2.33 m 위를 다림질이 6.29 m 지나갔다 (2.7 배). 사용자 보고는 "높이가 낮은 부품은 탑 표면이
이상했다" 였다. 다림질이 그 결함의 원인인지는 `[추정]` 이다 — 실측은 길이 비율까지이고 같은 부품을 다림질만 빼고 다시
출력한 대조가 아직 없다. 그래서 금지가 아니라 **기본 끔**이고, 사용자가 실물 비교 뒤 요청하면 켠다.
