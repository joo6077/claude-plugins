...
**외벽을 낮추는 프로파일은 `bridge_speed` 를 반드시 같이 쓴다.** 공통 프리셋은 `25` 지만 H2S 프리셋이 `50` 으로 덮어써서,
키를 안 쓰면 외벽만 `30` 이 되고 허공 위 구간은 `50` 으로 남는다 — 지지대 없이 허공에 거는 구간만 외벽보다 1.7 배 빠르다.
...
"아랫면 휘어진 부분이 밑으로 쳐지고 까끌까끌하다" 였다. Phase 4.3 게이트가 외벽 하향을 감지하면 `bridge_speed > 30` 을 FAIL 로 잡는다.
