...
## [2026-09-24] — 카이젠 사이클 (Phase 1 ~ 17 · Final) — 미검증 네 칸 · 원인 가르기 · 조용한 통과 막기

### 개요 (2026-09-24 사이클)

가지 `kaizen/2026-09-24`. Phase 1 ~ 17 과 Final 후속 계약 둘(`kaizen-0924-f1-harness-followups` · `kaizen-0924-f1-kit-followups`)이 모두 1 회차 QA 승인으로 끝났다.
Phase 마다 봉인 전 독립 검토를 두 회차 돌렸고, QA 뒤 교차 진단이 열일곱 Phase 를 다시 쟀다. 뒤집은 판정은 없다. Phase 7 · 8 · 9 · 11 은 DG-02 를 뜻 기준으로 FAIL 로 볼 근거가 있어
(연구 기록 새 소제목이 옛 소제목과 겹쳐 옛 줄에 같은 제목 경고가 생김) Final kit followups 가 구현을 고쳤다. 사이클을 관통한 주제는 셋이다 — 검증을 못 할 때 `[미검증]` 에
네 칸(막는 것 · 시도한 우회 · 통제 불가 사유 · 재검증 명령)을 적는다, 빨간 검사를 이번 변경 탓으로 단정하기 전에 원인을 가른다, 첫 칸만 읽거나 한 칸 못 읽으면 꺼지는 검사를 막는다.

### Phase 1 — 설계 가이드 (2026-09-24 사이클)

설계 가이드 두 편을 고쳤다. 스킬이 검증을 못 할 때 `[미검증]` 에 사유 한 줄만 붙이던 규칙을 네 칸(막는 것 · 시도한 우회 · 통제 불가 사유 · 재검증 명령)으로 바꾸고,
작업 자체를 못 한다고 결론 내리기 전에도 같은 네 칸을 먼저 적게 했다. 에이전트 가이드의 미검증 정책은 평가자가 이미 쓰던 `ENV` / `INVALID` 분류와 같은 말로 다시 썼다.
새로 짠 측정 스크립트가 0 이 아닌 값을 낼 때 손으로 답을 셀 수 있는 작은 입력으로 먼저 맞추는 「알려진 답 대조」 를 넣었다. 공식 문서를 다시 조회해 frontmatter 18 종,
내장 Explore 모델 상속, 세션 전체 스폰 수 상한 없음을 반영했고, 500 줄은 상한이 아니라 권고로 고쳤다.

### Phase 2 — contract (2026-09-24 사이클)

계약 스키마를 v5.5 로 올렸다. 구현이 만들 값은 미리 잴 수 없어 면제지만 그 값을 재는 명령의 준비 단계는 봉인 전에 돌리게 했다. 새로 짠 측정이 0 이 아닌 값을 내면
알려진 답 대조를 먼저 한다. 조건 수는 자동 포함 줄 · 금지 패턴 줄 · 해당 없음 줄을 빼고 기능 조건만 세어 단순 작업도 수를 지킬 수 있게 했다. 변경 범위 조건은 커밋 구간으로 재고,
여러 주체가 한 가지에 커밋하면 서명 줄로 내 커밋을 가린다. 셸이 없는 세션은 0 단계에서 멈추고, 계약 시각은 `date` 출력을 옮기고, 초안 끝에는 사용자가 할 일 한 줄을 적는다.

### Phase 3 — evaluator (2026-09-24 사이클)

평가 가이드를 v5.1 로 올렸다. 스프린트가 만든 것이 검사 스크립트 · 막는 훅 · 새 시험 파일이면 원본에서 나온 「위반 0」 은 그 검사가 살아 있다는 증거가 아니다. 평가자는 임시 사본으로
다섯 가지(첫 칸만 읽기 · 표에만 올린 시험 · 한 칸 못 읽으면 전체 꺼짐 · 셸마다 다른 대상 수 · 효과 증명)를 돌리고 리포트 `Check Artifacts` 블록에 남긴다. 구현 판정마다 지운 파일을 뽑아
`Deletions` 블록에 올린다. 스키마 v5.5 와 표기를 맞췄고 평가 시각은 `date` 출력을 옮긴다.

### Phase 4 — harness (2026-09-24 사이클)

커밋 안전 훅이 경로를 지정한 커밋(`git commit -o <경로>` · `-- <경로>`)도 검사한다. 그전에는 이 형태를 통째로 건너뛰어 그 경로 안의 대량 삭제가 그대로 실렸다. 피드백 저장 스크립트는
초안에 `project_hash` · `project_name` 을 요구하지 않는다. `/sprint` 는 내 경로만 싣는 커밋 절차, 병렬 작업일 때 워크트리, 빨간 검사의 원인을 이번 변경 · 남의 미커밋 · 기준 커밋 셋으로
가르는 코드 블록을 얻었다. V10 표 검사가 스킬 폴더 안 references 문서까지 본다. 카이젠 수집기의 폴더 이름은 한 곳에만 적고, 범위 밖 커밋을 막을 때 훅이 읽을 자리(계약의 `# sprint-scope` 블록)를 정했다.

### Phase 5 — flutter-toolkit (2026-09-24 사이클)

codegen 에 `--build-filter` 를 스스로 붙이지 않는다. 킷이 codegen 을 직접 돌리는 다섯 자리는 전후 삭제 수 블록으로 돌고, 늘어나면 한 번 더 돌린 뒤 첫 기준과 비교해 남은 삭제를 종료 코드로 드러낸다.
widget-inspector 에 관례 대조가 생겼고, 생성 측 `[미검증]` 아홉 자리가 네 칸을 요구한다. 자세한 내용은 `docs/kaizen/flutter-changelog.md` 2026-09-24 항목.

### Phase 6 — design-kit (2026-09-24 사이클)

공유 규약 `visual-change-protocol.md` 가 편집 전에 대상 화면 경로 · 한 문장 되말하기 · 같은 역할 기존 화면 2 개 이상의 관례 표를 남기게 한다. 재캡처에서 기준 표식이 바뀌었는지로 반영을 판정하고
스스로 고치기는 최대 3 회다. 승인 기록에 확정 구성 · 폐기한 대안 칸이 생겼다. DTCG 예시 · Style Dictionary 안내 · WCAG 2.2 표기를 바로잡았다. 연구 기록은 `docs/design/research-log.md` 2026-09-24 항목.

### Phase 7 — backend-kit (2026-09-24 사이클)

시각 필드를 순간 · 받는 사람 지역을 따라가는 벽시계 · 특정 지역에 묶인 벽시계 셋으로 나눈다(`docs/backend/fundamentals/database.md` 원칙 10 · Gotcha 18 · 19 · 감사 기준 두 행).
감사 기준 CDC 행의 exactly-once 서술을 at-least-once 와 consumer idempotency 요구로 바로잡았고 OAuth 2.1 인용은 draft-16, OpenAPI 는 3.2.1 로 올렸다.

### Phase 8 — infra-kit (2026-09-24 사이클)

빨간 CI 를 이번 변경 탓으로 단정하기 전에 원인부터 가른다(`docs/infra/platform/cicd.md` 원칙 7 · infra-guide Gotcha 14). 기준 커밋에서 이미 실패하던 검사도 통과로 적지 않는다.
kubeconform 스키마 판 번호를 고정값 대신 클러스터 판 번호 변수로 받고, 근거가 없던 OpenTofu state encryption 도입 판 번호 표기 네 자리를 뺐다.

### Phase 9 — rust-kit (2026-09-24 사이클)

rust-preflight 가 빨간 clippy · test 를 내 변경 · 남의 미커밋 변경 · 기준 커밋에서 이미 실패로 가른다(Step 3.5). rust-model 은 시각 필드 종류를 먼저 정하고 ORM 마다 다른 타입 대응을 따른다
(`docs/rust/data/sqlx-patterns.md` 원칙 6). rust-reviewer 의 미검증 기준 원본 사본을 현행 판으로 다시 맞췄다.

### Phase 10 — react-kit (2026-09-24 사이클)

렌더 증거 규약이 편집 전에도 돈다 — 되말하기 · 화면 대상 · 관례 표. 완료 직전에는 개발 서버 주소와 브라우저 주소를 대조하고 기준 표식으로 반영을 판정한다. `strictPort` 로 포트가 차 있으면 멈추고,
react-run · react-preflight 는 시험 결과를 passed · skipped 두 수로 적는다. react-l10n 은 `lingui extract --clean` 을 요청할 때만 돈다.

### Phase 11 — planning-kit (2026-09-24 사이클)

사용자가 버린 기능 · 설정 항목을 그 기능 PRD 의 비범위 절 한 곳에 네 칸 표로 적는다. 뒤 단계 스킬(plan-stories · plan-data-model · plan-flow)은 비범위 절을 읽고 겹치는 것을 만들기 전에 묻고,
plan-audit · planning-reviewer 가 폐기한 항목이 다시 들어갔는지 본다. `flows.md` · `data-modeling.md` 를 Mermaid 12.0.0 기준으로 현행화했다.

### Phase 12 — reflect-kit (2026-09-24 사이클)

Stop 훅이 다시 기록을 남긴다. codex-cli 가 훅의 `--full-auto` 인자를 거부해 한 달 가까이 아무것도 못 적었는데 stderr 를 버려서 원인이 로그에 없었다. 이제 `-s read-only` 로 부르고 실패하면
`.errors.log` 에 원인 한 줄을 남긴다. 분석기 세션에서 도는 훅은 아무것도 적지 않고, `/reflect-digest` 머리 첫 줄은 수집 상태다. 워크트리 안에서 불러도 로그 폴더는 본 레포 이름이다.

### Phase 13 — bambu-kit (2026-09-24 사이클)

MakerWorld 모델을 읽을 때 브라우저보다 JSON 주소를 먼저 부른다(화면 없는 브라우저는 403 으로 막혔다). 조용히 통과하던 검사 다섯 자리를 막았다 — 첫 칸만 읽던 값 박기 · G-code 대조,
표에도 실행 줄에도 없던 시험 파일 8 개, 비어 있는 옵션 목록이 키 검사를 끄던 것, 호를 빠뜨리던 G-code 길이 재기, 정답을 모르는 형상 측정.

### Phase 14 — onboarding-kit (2026-09-24 사이클)

셋업 가이드의 막는 요구마다 출처 · 막히는 것 · 우회 세 칸을 적고, 출처가 요구하지 않으면 막는 요구로 쓰지 않는다. 한 번도 안 돌던 게이트 시험 입력을 `evals.json` 에 등록하고
zsh · bash 두 셸로 대조하는 러너 `evals/run-gate-evals.sh` 를 더했다. 값이 든 `.env` 는 있는지만 보고 열지 않는다.

### Phase 15 — tone-kit (2026-09-24 사이클)

한국어 규칙에 K-11 「새 이름을 만들지 않는다」 를 관측 컨벤션으로 넣었다. `core-naming.md` §8 이름 게이트 넷이 표 칸에 적혀 붙여 넣으면 0 건을 내던 것을 코드 블록으로 옮기고,
확장자 변수는 zsh 가 나누지 않아 배열로 바꿨다.

### Phase 16 — api-kit (2026-09-24 사이클)

`/api-verify` 가 경로 간 불변식 판정 줄마다 양쪽 실제 값을 적고 한쪽 경로가 없으면 `판정 불가` 로 따로 센다. `/api-contract` 는 새 pin 을 사본에서 한 번 망가뜨려 FAIL 이 나는지 보고하고,
`/api-ui` 는 생성한 뷰어를 브라우저로 열어 콘솔 오류 · 항목 수 · 누르는 자리 크기를 잰다. `HURL_VARIABLE_` 접두 · `--secret` 가림 범위 · I-JSON `-0` 기재를 실측에 맞췄다.

### Phase 17 — howto-kit (2026-09-24 사이클)

`/howto-audit` 가 부모 셸에서 게이트 스크립트를 읽고 자식 셸에서 부르던 탓에 판정 줄이 0 이던 것을 고쳤다. 게이트를 부르는 네 블록이 스크립트를 플러그인 설치 경로 → git 최상위 폴더 →
마켓플레이스 설치본 순으로 찾고 못 찾으면 멈춘다. 러너는 사례를 zsh · bash · sh 세 셸로 대조한다(사례 16 → 33).

### Final — kaizen-0924-f1-harness-followups (2026-09-24 사이클)

커밋 안전 훅이 경로를 지정한 커밋과 `-i` 커밋에서 이름 바꾸기를 삭제로 세지 않는다. 피드백 저장본과 평가자의 사용자 교정 로그 조회가 워크트리에서도 본 레포 이름을 쓴다. 평가자는 리포트 시각을
저장 순간의 `date` 로 덮어쓰고 삭제 열거에서 이름 바꾸기 감지를 끈다. 계약 스키마 측정 예시는 상한 해석이 실패하면 멈춘다. validate-plugin 의 V 줄은 늘 판정 글자로 끝나고, 옛 값 검사는
marketplace.json 의 킷 폴더까지 읽는다(133 → 389 파일). 오케스트레이터와 참조 문서가 Phase 17 을 담고 CI 가 Phase 가 넘긴 새 시험 여섯을 돈다.

### Final — kaizen-0924-f1-kit-followups (2026-09-24 사이클)

킷 열두 곳의 후속 수정. 연구 기록 넷의 2026-09-24 소제목에 날짜를 붙여 같은 제목 경고를 없앴다(교차 진단 DG-02 해소). react-init 이 `strictPort` 를 실제 프로젝트에 넣고,
reflect-kit 수집 상태가 마지막 기록 뒤의 Stop 실패를 경고하며, bambu-kit 완료 검사가 enum 빠진 목록을 `[미검증]` 으로 남긴다. api-ui 는 서버를 뒤에서 띄우고 판정 줄로 포트 충돌을 가르며,
howto-kit 러너가 `sh` · `shell` · `zsh` 펜스도 센다.

### Final — kaizen-0924-final (2026-09-24 사이클)

Phase · followups 계약 열아홉의 상태와 QA 리포트를 실었고, 처리 배정표 Phase 행 일흔넷에 계약 슬러그와 QA 판정을 채웠다. 열일곱 Phase 의 평가자 · 계약 피드백에 교차 진단 결론과 측정 구멍을 적었다.
문서 사이트 페이지 마흔넷을 원본 전체로 다시 만들고(Claude 28 쪽 · Codex 16 쪽) 첫 화면 harness 다섯 항목 제목을 원본 판 번호로 고쳤다. 전역 피드백 가운데 500 개를 넘는 가장 오래된 것은
지우지 않고 보관 폴더로 옮겼다. 메모리 승격 후보 · evals 점검 · 실패 횟수 · 사이클 상태 · 감사 기록 · 옛 값 등록부 · 릴리스 계획을 이번 사이클로 채웠다. 판 번호는 올리지 않았다 — 릴리스 계획이 킷마다 단계를 적는다.
