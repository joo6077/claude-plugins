...
2. **ISO/IEC/IEEE 26514·26515 의 절차 작성 세부 조항**은 공개 OBP 에서 정의·개요만 확인했다. 전문은 유료 접근이 필요할 수 있다. **2026-09-10 재시도도 확인 실패** — `iso.org` 가 5.5KB 스텁만 응답했다 (접근 차단이지 부재가 아니다). 무료로 인용 가능한 대체 근거(DITA · Google · Microsoft)를 `docs/howto/procedure-standards.md` 에 확정했다.
