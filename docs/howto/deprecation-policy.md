...
문구도 `G4_DEPRECATION FAIL` 로 잡힌다. deprecation 주장이 아닌데 그렇게 분류된다.

다만 게이트가 요구하는 것은 **"그 주장에 출처를 달라"** 이고, 보존 기간도 출처가 있어야 하는
사실 주장이다. 그래서 **동작 자체는 방어 가능하고 분류 라벨이 부정확한 상태**로 남겨 둔다.
`종료 예정` · `폐지 예정` 단독형은 여전히 미탐이다 (기존 갭, 이번 범위 밖).

다음 카이젠 사이클에서 G4 를 "deprecation 주장" 이 아니라 "출처가 필요한 상태 변경 주장" 으로
재정의할지 판단한다.
