...
## 7.1 잔여 오탐 — 고치려다 구멍을 낼 뻔했다 (2026-09-10)

QA 가 적대적 탐색으로 찾았다. **"30일 지난 데이터를 삭제 예정입니다"** 같은 데이터 보존 안내가
`G4_DEPRECATION FAIL` 로 잡혔다.

**첫 시도는 과교정이었다.** 원인이 `- 확인:` 줄까지 주장으로 세는 것이라고 보고, 그 줄을 주장
탐지에서 **통째로 면제**했다. 오탐은 사라졌지만 QA 가 사각지대를 실측으로 보였다.

```text
- 확인: 이 기능은 폐지 예정이니 서두르라는 안내가 뜬다
- 출처: https://example.com/docs/api-keys   (deprecation 과 무관한 문서)

면제 상태   G4_DEPRECATION PASS unsourced_claims=0   ← 조작된 주장이 통과한다
```

`확인:` 에만 주장을 적으면 **출처와 무관하게 항상 통과**한다. 게이트의 목적이 "출처보다 강한
주장 금지" 인데 한 필드를 면제하는 것은 **목적과 반대 방향**이다. 되돌렸다.

**올바른 해법은 이 킷의 기존 저작 규약이다.** 화면이 띄우는 문구를 옮겼다면 `출처:` 줄에 그
근거를 주석으로 단다.

```text
- 확인: `30일 지난 데이터를 삭제 예정입니다` 안내가 보인다
- 출처: https://example.com/docs/retention (조회 2026-09-10) — 문서에 "삭제 예정" 명시

  → G4_DEPRECATION PASS unsourced_claims=0
```

그러니까 **애초에 오탐이 아니었다.** 직전 QA 도 "동작 자체는 방어 가능하고 분류 라벨이 부정확한
상태" 라고 판정했는데, 그것을 오탐으로 읽은 것이 이 사이클의 실수다.

### 구멍을 만들지 않았다 — 주장은 어느 필드에 와도 잡힌다

면제를 되돌렸으므로 **주장이 스텝 헤더에 오면 여전히 탐지된다.** 다른 필드도 마찬가지다 —
`무엇을:` · `동작:` · `값:` · `안 보이면:` · 자유 서술, 그리고 되돌린 `확인:` 까지 전부 스캔한다.

증거는 SC-02 의 **양성 3 종**이다. 셋 다 주장을 **스텝 헤더**에 두고 출처는 뒷받침하지 않는데,
면제 철회 후에도 전부 `FAIL` 로 잡힌다.

| 픽스처 | 토큰 | 주장 위치 | 결과 |
| --- | --- | --- | --- |
| `fail-g4-korean-delete-unsourced.md` | `삭제 예정` | 스텝 헤더 | `G4_DEPRECATION FAIL unsourced_claims=1` |
| `fail-g4-korean-shutdown-unsourced.md` | `종료 예정` | 스텝 헤더 | `G4_DEPRECATION FAIL unsourced_claims=1` |
| `fail-g4-korean-abolish-unsourced.md` | `폐지 예정` | 스텝 헤더 | `G4_DEPRECATION FAIL unsourced_claims=1` |

그리고 `값:` 필드에 주장을 숨긴 적대적 픽스처도 `FAIL` 한다 — 다중 스텝 중 1 건만 위반일 때
`unsourced_claims=1` 로 정확히 센다.

반대로 **오탐 대조 3 종**(데이터 보존 안내 · 설치 완료 서술 · 계정 삭제 액션)은 전부 `PASS` 다.
탐지 범위를 넓혔지만 정상 절차를 깨지 않았다는 뜻이다.

같은 사이클에서 진짜 미탐 하나는 메웠다. `종료 예정` 단독형이 토큰 목록에 없어 통과하고 있었다.
**`종료` 단독은 넣지 않았다** — "설치가 종료됩니다" 같은 완료 서술을 오탐한다.

7 케이스는 `howto-kit/evals/fixtures/` 에 전부 픽스처로 있고 `evals.json` 에 등록됐다 (16 케이스).

**교훈 둘.** (1) 오탐을 만나면 토큰부터 손대지 말고 **어느 줄을 세고 있는지** 먼저 보라.
(2) 그리고 **필드를 통째로 면제하지 마라** — 게이트의 목적과 반대 방향이면 그것은 수정이 아니라
구멍이다. 앞선 QA 가 "방어 가능" 이라고 판정한 동작을 고치려 들 때는 특히 그렇다.
