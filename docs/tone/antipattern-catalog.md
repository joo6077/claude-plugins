...
규칙 강도는 세 등급이다. `MUST` (공식 문서가 금지·강제하거나 예외가 없음) / `SHOULD` (권고 수준, 명시 예외 존재) / `관측 컨벤션` (공개 근거 없음, 실측만 존재).

---

## 원칙

### 1. A 이름 번역 주석 `[코어]` `[한국어]` — MUST 삭제

식별자 이름과 타입이 이미 담고 있는 의미를 주석이 한국어로 되풀이하는 것. 주석을 지워도 정보 손실이 0이다.
...
처리는 삭제다. 예외는 하나뿐 — **이름이 약해서 주석이 필요한 경우, 주석이 아니라 이름을 고친다.** 주석만 지우고 이름을 그대로 두면 의미가 사라진다. hex 값의 alpha 퍼센트처럼 값에서 사람이 곧바로 읽어낼 수 없는 메타 정보는 A 가 아니다.

Effective Dart 의 등급은 `AVOID` 이고 이 카탈로그는 이를 `MUST` 로 승격했다. 승격 사유는 실측 최다 빈도(48파일 전역, 약 85건)와 제거 시 손실 0 이다.

> **출처:** [Effective Dart — documentation](https://dart.dev/effective-dart/documentation) · 프로젝트 실측 (57 파일 / 약 85건, 2026-04~06)

### 2. B 템플릿 마커 `[코어]` — MUST 삭제

파일마다 같은 위치에 기계적으로 반복되는 고정 라벨. `//상태`, `//위젯 구현부`, `//뷰모델` 류로, 그 파일이 무엇인지는 클래스 선언이 이미 말한다.

판정 신호 — 동일 주석 줄의 코퍼스 전역 빈도. 생성 파일을 반드시 제외한다:
...
빈도 상위에 뜨는 짧은 라벨이 후보다. 처리는 삭제이고 **예외는 없다.** 파일 구조를 알려주는 역할이 필요하면 그것은 파일 분할이나 이름으로 해결할 문제이지 마커로 해결할 문제가 아니다.
...
### 3. C 메서드명 반복 doc `[코어]` — SHOULD 삭제

메서드 이름을 한국어 문장으로 풀어 쓴 doc 주석. `handlePressStart()` 위의 `/// 눌림 시작 상태를 처리합니다.` 가 전형이다. 번역투 종결형(`~을 처리합니다`, `~하도록 합니다`)이 같이 따라온다.
...
처리는 삭제. 예외는 **public API 계약** 과 **부수효과 순서가 호출자에게 중요한 경우** 로, 이때는 doc 을 남기되 이름의 반복이 아니라 계약을 쓴다. 파라미터·반환값 블록의 표기 형식(`- 반환값: 없음` 을 유지할지 등)은 이 카탈로그가 아니라 어댑터의 정책 상수가 정한다 — 어댑터가 유지를 지시하면 C 로 판정하지 않는다.

> **출처:** [Effective Dart — documentation](https://dart.dev/effective-dart/documentation)

### 4. D 프레임워크 기본 설명 `[코어]` — MUST 삭제

프레임워크 공식 문서에 이미 적힌 기본 동작을 코드 옆에 옮겨 적은 주석. `SizedBox` 앞의 `// 크기 지정`, `ClipRRect` 앞의 `// borderRadius 클리핑` 같은 것이다.

정규식으로 분리되지 않는다. 판정은 리뷰로 한다 — **주석이 설명하는 내용이 그 API 문서 첫 문단에 있으면 D 다.**

처리는 삭제이되, "왜 이 선택을 했는가"는 남긴다. 남는 그 한 줄은 D 가 아니라 H 로 승격된다. 프레임워크 기본 동작 서술은 정보량이 0일 뿐 아니라 프레임워크 버전이 오르면 거짓이 되므로 유지 비용이 음수다.
...
### 5. E 접두사 fallback 변수명 `[코어]` — 관측 컨벤션 (실측 9건 / 4파일)

`effective*` / `resolved*` 접두사로 "기본값이 적용된 값"임을 이름에 박는 것. `??` 연산자가 이미 fallback 을 표현하므로 접두사는 역할이 아니라 계산 과정을 드러낸다.
...
처리는 삭제가 아니라 **도메인·역할명으로 개명** 이다. `toolbarGradient`, `visibleChildren` 처럼 그 값이 무엇인지로 바꾼다. 개명은 선언만 고치면 끝나지 않는다 — 호출부 전부를 함께 바꿔야 한다.

이 카테고리는 **공개 통계 근거가 없다.** 2026-08 조사에서 접두사별 LLM 과대표집 통계는 공개 1차 문헌 어디에도 확인되지 않았다. 그래서 강도를 `관측 컨벤션` 으로 낮춰 표기하고 각주를 붙이지 않는다. 실측은 유효한 국지 근거이므로 규칙을 지우지도 않는다.

> **출처:** 프로젝트 실측 (57 파일 / 9건 · 4파일, 2026-04~06)

### 6. F 구분선 블록 `[코어]` — MUST 삭제

`// ---` 를 수십 칸 늘여 만든 시각적 구획. 한 파일에 14개까지 쌓이면 구분 기능 자체가 사라진다.
...
처리는 삭제. 대체 수단은 빈 줄과 의미 있는 함수·클래스 이름이다. 구분선이 필요하다고 느껴진다면 그것은 파일이 너무 길다는 신호이지 선이 필요하다는 신호가 아니다. 예외는 코드 생성기가 만드는 배너다 — 생성 파일은 대상에서 제외한다.
...
### 7. G 자명 구조 라벨 `[코어]` `[한국어]` — SHOULD 코드 구조로 흡수

위젯 트리나 블록 위에 붙은 짧은 한국어 라벨. `// 제목`, `// 서버 이름`, `// 좌측: leading 또는 기본 뒤로가기` 처럼 바로 아래 코드가 그대로 말하는 내용이다.
...
처리는 삭제가 아니라 **흡수** 다. 라벨이 담던 의미를 변수명(`serverNameText`)이나 추출한 위젯 이름으로 옮긴 뒤 라벨을 지운다. 지우기만 하면 정보가 없어진다.

예외는 섹션 라벨의 경제성 규칙이다 — 라벨이 **2개 이상의 선언을 묶고** 그 묶음의 축을 이름으로 표현할 수 없을 때만 남긴다. 선언 1개짜리 섹션 라벨, 그리고 라벨과 변수명이 같은 뜻인 경우는 G 다.

> **출처:** [Effective Dart — style](https://dart.dev/effective-dart/style) · 프로젝트 실측 (57 파일 / 약 35건 · 15파일 이상, 2026-04~06)

### 8. H 좋은 주석 `[코어]` — MUST 보존 (위반 아님)

**H 는 안티패턴이 아니다.** 코드만 읽어서는 복원할 수 없는 정보를 담은 주석이며, 이 카탈로그에 들어 있는 이유는 A~G 를 일괄 삭제할 때 함께 지워지는 사고를 막기 위해서다. H 를 위반 목록에 넣는 것이 이 카탈로그의 최대 오용이다.

보존 기준은 셋이다. 셋 중 하나에 해당하면 남긴다.
...
### 9. I 과도한 빌더 분할 `[어댑터:dart-flutter]` — SHOULD 재구성

`build()` 가 `_buildIcon → _buildContent → _buildTextBlock → _buildIconTextLayout` 처럼 다단 호출 체인으로 흩어진 것. 이름이 본문보다 정보를 더 주지 못하는데 호출 단계만 늘어난다.
...
처리는 "무조건 인라인" 도 "무조건 분할" 도 아니다. 분할이 정당한 조건은 셋이다 — **의미 경계가 실제로 다른가 / 재사용되는가 / 독립 테스트 대상인가.** 셋 다 아니면 인라인한다. 셋 중 하나라도 해당하면 헬퍼 메서드가 아니라 별도 위젯으로 뺀다. 구체적 임계치(줄 수, 헬퍼 개수, 추출 위치)는 이 카탈로그가 아니라 어댑터가 정한다.

이 카테고리는 코어로 올리지 않는다. 다른 스택으로 옮기려면 그 스택에서 위반이 실제로 관측된 뒤에 어댑터를 추가한다.
...
### 10. J 중복 구현 `[코어]` — SHOULD 공통 요소로 수렴

프로젝트에 이미 있는 공통 컴포넌트를 쓰지 않고 그것이 감싸던 저수준 조합을 호출부에서 다시 조립하는 것. 눌림 애니메이션 공통 위젯이 있는데 제스처 감지기와 스케일 애니메이션을 직접 엮는 식이다.

**판정에 프로젝트 파라미터가 필요하다.** 공통 위젯 목록과 각 위젯이 감싸는 저수준 요소를 모른 채로는 판정할 수 없다. grep 은 그 목록을 채운 뒤에만 만들어진다:
...
처리는 공통 요소로 수렴. 예외는 **의도된 divergence** 다 — 공통 위젯으로 표현할 수 없는 요구라서 갈라놓은 것이면 그 이유를 주석으로 남긴다. 그 주석은 H 이므로 나중에 삭제 대상이 되지 않는다.

수렴 방향을 뒤집지 마라. 호출부 6곳을 공통 위젯에 맞추는 것이 원칙이고, 공통 위젯에 파라미터를 6개 늘려 호출부를 수용하는 것이 아니다.
