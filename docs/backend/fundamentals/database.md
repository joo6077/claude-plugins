...
### 10. 시각은 종류부터 나누고, 종류마다 저장 형태를 정한다

"전부 UTC(세계 표준시)로 저장한다" 는 한 순간에만 맞는 처방이다. 반복 일정·영업시간·알림 시각처럼 사람이 정한 시계 값(벽시계)을
순간 하나로 바꿔 저장하면, 시간대가 바뀌거나 서머타임이 드나들 때 사람이 정한 시각과 어긋난다. 시각 필드마다 아래 셋 중 무엇인지부터 정한다.

| 시각 종류 | 예 | 저장 형태 (PostgreSQL) | 순간이 필요할 때 |
| --- | --- | --- | --- |
| 순간 | 결제 시각, 생성 시각 | `TIMESTAMPTZ` | 그대로 쓴다 |
| 받는 사람 지역을 따라가는 벽시계 | 어디에 있든 현지 06:30 에 울리는 알림 | `TIME` · `TIMESTAMP` (시간대 없음) | 받는 사람의 지금 시간대로 계산한다 |
| 특정 지역에 묶인 벽시계 | 한 지점의 영업시간, 한 도시의 행사 | `TIME` · `TIMESTAMP` + 시간대 이름 칸 | 레코드에 적힌 시간대로 계산한다 |

- 시간대 이름 칸에는 IANA 시간대 식별자(`Asia/Seoul` 처럼 지역 이름으로 적는 시간대 이름)를 둔다.
- `TIMESTAMPTZ` 는 입력 시간대를 UTC 로 바꿔 저장하고, 명시됐거나 추정한 원래 시간대는 남기지 않는다. 사용자가 고른 시간대가 업무 정보면 별도 칸에 둔다.
  `TIMESTAMP` 는 시간대 표시를 무시하고 날짜·시각 값만 저장한다.
- 받는 사람 지역을 따라가는 벽시계는 RFC 5545 의 floating time(시간대 없는 지역 시각)이다. 보는 사람의 시간대와 상관없이 같은 시·분·초를 뜻하므로
  시간대가 다른 사람은 서로 다른 UTC 순간에 참여한다. RFC 는 이 형태를 합리적일 때만 쓰고, 대부분은 UTC 나 시간대 참조로 시각을 고정하라고 한다.
- 벽시계를 순간으로 바꾸는 규칙을 계약에 적는다 — 어느 시간대를 쓰는지, 서머타임 전환으로 같은 시각이 두 번 오거나 아예 없을 때 어떻게 하는지.
  RFC 5545 는 두 번 오는 시각을 첫 번째로, 없는 시각을 전환 전 오프셋으로 해석한다.
...
| 벽시계 뜻의 값(반복 일정·영업시간·알림 시각)을 `TIMESTAMPTZ` 하나로만 저장 | 시간대가 바뀌거나 서머타임이 드나들면 사람이 정한 시각과 어긋난다. |
| 시간대·나라를 코드 상수나 한 나라 기본값으로 박음 | 다른 지역 사용자와 그 지역의 서머타임 규칙을 표현하지 못한다. |
