...
비율(Ratio)은 디자인에서 요소 간의 관계를 수학적으로 정의하는 기본 도구다. 임의의 수치 대신 체계적인 비율을 적용하면 다음과 같은 이점을 얻는다:

1. **시각적 조화** — 수학적 관계에 기반한 크기와 간격은 인간의 시각 체계가 자연스럽게 인지한다. 연구에 따르면 황금비에 가까운 비율의 이미지가 뇌의 특정 세포군을 활성화시킨다.
2. **일관성** — 하나의 비율 체계에서 파생된 값들은 서로 조화롭게 어울리며, 디자인 시스템 전체에 통일감을 부여한다.
3. **확장성** — 비율 기반 레이아웃은 다양한 화면 크기에서 비례를 유지하며 반응형 디자인의 핵심이 된다.
4. **의사결정 효율** — "왜 이 크기인가?"에 대한 답이 명확해지므로 디자인 리뷰와 협업이 빨라진다.
...
- 반응형 웹에서 모든 뷰포트 크기에 황금비를 완벽히 적용하기 어렵다.
- 행간 계산 시 줄 길이(line length)에 따른 가독성 조정이 필요하다.
...
- 모듈러 스케일은 도구이지 마법이 아니다. 스케일에서 벗어난 값이 시각적으로 더 적합하다면 그 값을 사용하라.
- 반응형 환경에서 황금비를 모든 뷰포트에 적용하려 하지 말라. 브레이크포인트별로 비율을 조정하라.
- 8px 그리드 시스템과 모듈러 스케일 값이 충돌할 경우, 8의 배수로 라운딩하는 것이 실용적이다.
- 디바이스 픽셀 밀도를 고려하여 2x 기준(xhdpi, ~430ppi)을 표준 작업 해상도로 채택하라.
