...
howto_gate() {
  ...
  stats=$(awk -v verbs="$HOWTO_VERBS" -v hedge="$HOWTO_HEDGE" -v dep="$HOWTO_DEP" '
    ...
    {
      if ($0 ~ /\[추정\]/) guess = 1
      # `- 확인:` 줄도 주장으로 센다. 2026-09-10 에 이 줄을 면제해 봤다가 되돌렸다 —
      # 면제하면 `확인:` 에만 조작된 deprecation 주장을 적을 때 출처와 무관하게 **항상**
      # 통과하는 사각지대가 생긴다 (QA 적대적 탐색으로 실측). 게이트의 목적이
      # "출처보다 강한 주장 금지" 이므로 한 필드를 통째로 면제하는 것은 목적과 반대다.
      # 화면이 띄우는 문구를 옮긴 것이라면 `출처:` 줄에 그 근거를 주석으로 달면 통과한다
      # (예: `출처: <URL> (조회 …) — 문서에 "삭제 예정" 명시`). 그것이 이 킷의 저작 규약이다.
      if ($0 ~ dep) dep_claim = 1
    }
