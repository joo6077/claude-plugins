...
## Step 2c. 버전 현행성 확인 (버전 리터럴 SSOT)

rust-kit 문서에 적힌 크레이트 버전은 **작성 시점 스냅샷**이다. 아래 표가 이 킷에서 버전 값을
적는 **유일한 자리**이며, 다른 스킬·기준 문서는 이 Step 을 인용만 하고 자체 표를 만들지 않는다.

| 크레이트 | 조회 시점 최신 (crates.io · 2026-09-24) | rust-kit 문서 예시가 전제하는 버전 |
| -------- | --------------------------------------- | ---------------------------------- |
...
| `rust-i18n` | 4.2.2 | 3 |
| `opentelemetry` · `opentelemetry-otlp` · `tracing-opentelemetry` | 0.33.0 · 0.33.0 · 0.34.0 | 0.31 · 0.31 · 0.32 |
| `mockall` | 0.15.0 | 0.13 |
| `rstest` | 0.27.0 | 0.26 |
| Rust stable | 1.98.1 | 1.88.0 (`rust-init` 고정 예시) |

**해석 규칙 — 프로젝트에 고정된 버전이 우선이다.**

- 기존 프로젝트의 `Cargo.toml` / `Cargo.lock` 이 하위 버전에 고정돼 있으면 **그 버전을 그대로
  따른다.** 문서에 최신이 적혀 있다는 이유로 버전을 올리지 마라 — major 이동은 migration 비용을
  만들고, 이 킷은 그 비용을 감수할 근거를 갖고 있지 않다.
- 신규 프로젝트 스캐폴딩(`rust-init`)에서는 위 최신값을 **후보로 제시**하되, feature 이름·기본값이
  major 마다 달라질 수 있으므로 착수 시점에 공식 문서(docs.rs / CHANGELOG / Context7)로 확인한
  뒤 확정한다. 확인 없이 feature 목록을 그대로 옮겨 쓰지 마라.
- 위 표에 없는 크레이트의 버전을 문서에 새로 적을 때는 **조회 근거(URL + 조회일)** 를 함께 남긴다.
  근거 없이 버전 번호를 적는 것은 날조다.
- 감사 시에는 "구버전 사용" 자체를 FAIL 로 쓰지 마라. FAIL 대상은 **문서와 코드가 어긋난 경우**
  (예: 코드가 `axum 0.8` 인데 라우트가 `:id` colon 문법) 다.
