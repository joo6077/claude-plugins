...
def main() -> None:
    ...
    print("[1/5] 글로벌 feedback 수집 중...", file=sys.stderr)
    global_fb = collect_global_feedback()

    print("[2/5] Hub 외부 프로젝트 수집 중...", file=sys.stderr)
    hub_projects = collect_hub_projects(args.hub_dir)

    print("[3/5] followup 문서 수집 중...", file=sys.stderr)
    followups = collect_followup_docs()

    print("[4/5] 현재 레포 sprint-contract 이력 수집 중...", file=sys.stderr)
    local_contracts = collect_recent_local_contracts()

    validate_output: str | None = None
    if not args.skip_validate:
        print("[5/5] validate-plugin 스냅샷 실행 중...", file=sys.stderr)
        validate_output = run_validate_plugin()
    else:
        print("[5/5] validate-plugin 스냅샷 건너뜀 (--skip-validate)", file=sys.stderr)

    content = render_data_pool(
        global_fb, hub_projects, followups, local_contracts, validate_output
    )
