...
- **변경 범위(scope) 조건에 `git diff` 를 자유 서술로 적지 마라 — 표준형 5 요소를 다 채워라.** (1) `Given:` 상태 전제 (2) 경로 한정 pathspec (3) 생성물 제외 pathspec (`':(exclude)*.g.dart'` 등) (4) "정확히 일치" 인지 "포함" 인지 (5) **상한 ref — `HEAD` 금지** — 상한 ref 는 커밋 구간을 재는 조건에만 요구한다 (커밋 전 두 상태 `git diff HEAD` · `--cached` 에는 상한이 없다). 그리고 **계약 작성 시점에 그 명령을 1 회 실행해 baseline 을 서술 섹션에 남겨라.** 미커밋 codegen 산출물이 diff 에 섞여 scope 조건이 깨진 것이 AR-01 재발 3 회의 직접 원인이다 (E3 승급). contract-design-guide §Diff-Scope Oracle 표준형 참조
- **diff 구간 상한에 `HEAD` 를 쓰지 마라.** `HEAD` 는 평가 시점마다 움직여서, 봉인 이후 브랜치에 들어온 **무관한 작업**이 화이트리스트 위반으로 잡힌다 — 스프린트가 자기가 만들지 않은 변경 때문에 FAIL 한다. 상한은 이 스프린트가 소유한 ref (머지 커밋 또는 브랜치 tip) 로 하고, 해시를 박는 대신 **해석 명령을 계약에 적어라** (`sprint_head` 스니펫은 contract-schema.md §커밋 구간 상한 이 SSOT). 해석 실패(`UNRESOLVED`) 시 조용히 `HEAD` 로 떨어지지 마라. 실측 재발 2 회: `mcp-dynamic-pull-collision` 의 SC-02 가 `1752e9a..HEAD` 로 재는 바람에 봉인 뒤 병합된 0.14.1 이 화이트리스트 밖 파일을 건드려 FAIL 했고, 머지 커밋까지로 좁히자 4 경로 전부가 원 화이트리스트 안이었다 — 조건도 구현도 옳았고 상한만 틀렸다
- **재보지 않은 수치를 오라클로 잠그지 마라.** 값을 고정하는 조건(`== 0` · `>= 1500` · `정확히 N 행`)과 열거 집합은 **봉인 전에 1 회 실측**한다. 못 쟀으면 `[미실측]` 을 붙이고 Step 6.6 으로 가지 마라 — Step 6.5 (5) 가 막는다. 구현이 **만들어 낼** 값(신규 필드명 · 새 파일 경로)은 면제이며, 기준은 "구현 전에도 관측 가능한가" 다. 실측 재발: RS-01 이 «`totalTools` 가 `0`» 을 잠갔으나 결함이 내는 값은 `37` 이었고, PASS 집합이 공집합이라 개정이 유일한 출구가 됐다. 그 개정은 집합 비교상 필연적으로 `relaxing` 이라 사용자 승인을 강제한다 — 잘못 쓴 한 줄이 사용자 시간을 쓴다. contract-schema.md §미실측 오라클 봉인 금지 참조
...
- **사이드카 amendment 의 direction 을 자기신고하지 마라 — 집합형은 계산하라.** 경로 화이트리스트·파일 열거·대상 목록은 원 집합과 개정 집합을 `comm` 으로 비교해 direction 을 산출한다. 3 경로 → 5 경로는 계산상 `relaxing` 이라 "범위 조정" 이라 부를 여지가 없다. **앵커가 없다고 `unknown` 으로 적지 마라** — direction 과 consent 는 별개 축이고, `narrowing · unanchored` 는 정상적으로 PASS 근거다. 앵커 부재를 이유로 방향까지 무효화한 것이 사이드카를 무력화해 본문 직접 편집을 유발한 구조다 (contract-schema §Amendment 사이드카). **오라클(베이스라인 · 제외 pathspec · 측정 명령)을 바꾸는 amendment 는 측정 집합을 `amend_direction_oracle` 에 넣는다** — 측정 집합을 `amend_direction` 에 넣으면 극성이 뒤집혀 `relaxing` 이 `narrowing` 으로 적힌다 (실측 2026-09-08 howto-kit A-01). 정의는 스키마가 SSOT 다
...
**셸 실행 도구가 없는 세션이면 파일 쓰기 도구가 있어도 여기서 멈춘다 (2026-09-24 추가).** 이 스킬의 확인 단계(Step 0 · 0.5 · 6.2 · 6.5 · 6.6 · 6.7)는 전부 명령 출력으로 판정하므로 대체 경로가 없다. 파일 쓰기 도구만 없으면 멈추지 말고 셸로 파일을 쓴다. 멈출 때는 첫 줄에 없는 도구 이름을 적고 `../../docs/guides/skill-design-guide.md` §3.7 의 네 칸(막는 것 · 시도한 우회 · 통제 불가 사유 · 재검증 명령)을 채운 뒤 멈춘다. 셸이 없으면 「막는 것」 칸에 실행한 명령과 실패 출력을 적을 수 없으니, 대신 이 세션에서 쓸 수 있는 도구 목록을 적는다. 재검증 명령은 「도구를 켠 세션에서 이 스킬을 다시 부른다」 다. 계약을 짐작으로 쓰지 마라. 실측(2026-09-21): 도구가 하나도 없는 세션에서 이 스킬이 0 단계에서 멈췄다 — 멈춘 것은 옳았고, 이 문단은 그 동작을 규칙으로 못박는다.
...
**조건 작성 preflight — 통합 결함 태그 어휘.** DRAFT 를 제시하기 전에 조건마다 10 종 태그의
작성자 측 자문을 확인한다. 어휘 정의는 `harness/references/contract-schema.md`
§조건 작성 preflight 가 **SSOT 다** — 여기서 재정의하지 말고 그 표를 적용하라. 평가자가
붙이는 태그와 **같은 어휘**이므로, 평가에서 나온 태그가 다음 계약의 작성 자문으로 그대로
되먹여진다.
...
**복잡도별 조건 수 가이드 — 기능 조건만 센다 (2026-09-24 개정):**
...
아래 섹션은 **모든 계약에 자동 포함**된다. 조건을 지우거나 ID 를 바꾸지 않는다.
...
DRAFT 끝은 `사용자가 할 일: 없음` 또는 `사용자가 할 일: <한 줄>` 로 맺는다 (2026-09-24 추가). 절차가 길어도 사용자에게 남는 일은 한 줄로 드러나야 한다. 실측(2026-09-19): 한 줄 수정이 계약 · QA 절차에 묻혀 사용자가 「그래서 내가 뭘 하면 되냐」고 물었다.
...
- `created` — 본문을 저장하는 순간 `date '+%Y-%m-%d %H:%M'` 를 돌려 그 출력을 옮긴다. 짐작해 적지 마라 (2026-09-24 추가).
  ...
  피드백에 적는 시각도 같다. `locked_at` 은 Step 6.6 이 같은 명령으로 채운다
...
- 모든 카테고리에 최소 1개 조건 필수. 해당 없으면 `- [ ] XX-00: N/A (사유)` — **사유를 괄호에 반드시 적는다.**
  ...
  두 마커의 경계는 `../../docs/guides/qa-evaluation-guide.md` §Canonical Unverified-Evidence Protocol 이 SSOT 다
...
**(5) 미실측 오라클 — blocking 이다 (v5.4).** `[미실측]` 이 계약에 남아 있으면 봉인하지 않는다.
그 표기는 "이 값을 아직 안 재봤다" 는 뜻이라, 봉인하는 순간 개정 없이는 벗어날 수 없는 조건이 된다.

```bash
grep -nF '[미실측]' "$CF" && echo "BLOCKED 미실측 오라클 — 실측해 값을 확정한 뒤 다시 6.5 를 돌려라" \
  || echo "OK 미실측 0 건"
```

### 6.6. 계약 봉인 (E3)

**봉인 전에 Step 7 자기진단과 Step 8 교차 진단을 먼저 돌리고, 지적을 조건에 반영한 뒤 6.5 를 다시 통과시킨 다음 봉인한다.** 봉인 뒤에 교차 진단을 하면 지적을 조건 원문에 반영할 길이 사이드카뿐이다 (실측 2026-09-19: 봉인 뒤 교차 진단 1 회 → 사이드카 개정 2 건, 봉인 전 교차 진단 1 회 → 구현 전에 조건 6 곳 보강). 봉인은 구현 착수 직전의 마지막 단계다.
...
- **병합할 때 스쿼시를 쓰지 마라.** 이 레포는 세 방식(병합 커밋 · 스쿼시 · 재배치)을 모두
  ...
  `gh pr merge --merge` (병합 커밋) 또는 `--rebase` (재배치)로 병합한다 — 둘 다 봉인 커밋의
  파일 1 개를 지킨다 (교차 진단이 임시 복제본에서 세 방식을 실제로 돌려 확인했다).
  **재배치의 위험은 다른 데 있다** — `main` 이 앞서 있으면 커밋 해시가 바뀌어 QA 리포트에
  인용한 해시가 `main` 의 조상이 아니게 된다. 기능은 살고 인용만 죽으므로, 재배치로
  병합했으면 리포트의 해시를 새 값으로 갱신한다.
  이 레포는 스쿼시를 설정으로 껐다 (`allow_squash_merge=false`, 2026-09-24)
- **옛 계약에 소급으로 만들어 넣지 마라.** 봉인 커밋이 없는 계약은 `SEAL_ABSENT` 와 같은 급으로
  다룬다 — 경고이지 실패가 아니다. 없던 원문을 있는 것처럼 만드는 행위다

### 7. 자기진단

1. 구조화 체크리스트 실행 — **모든 항목은 「문제가 있다」 가 true 다 (2026-09-24 추가).** 문항이 「했는가」 와 「빠졌는가」 로 섞여 있으면 true 가 좋은 뜻인지 나쁜 뜻인지 갈려 카이젠이 반복 실패를 셀 수 없다. 실측(2026-09-24 21:49 기준, 최근 계약 피드백 40 건): 그 항목이 있는 31 건 중 20 건이 `nfr_coverage: true` 였는데 문항이 「반영되었는가」 라 대부분 「반영했다」 는 뜻으로 적혔다. 항목 이름은 피드백 키 호환 때문에 그대로 둔다:
   ...
   - `nfr_coverage`: 해당 기능에 걸린 비기능 요구사항(성능 · 보안 · 접근성)이 조건에서 빠졌는가? (이름과 달리 true = 빠졌다)
   ...
   - `diff_oracle_nonstandard`: 변경 범위 조건에서 표준형 5 요소(상태 전제/경로 한정/생성물 제외/기대 집합/상한 ref) 중 하나라도 빠졌는가?
   - `evidence_artifact_missing`: `[goal]` 조건이 참조하는 증거 기록물의 경로가 조건에서 빠졌는가?
   - `section_header_unclassified`: Step 6.5 게이트가 허용 밖 헤더나 서술 섹션의 조건 줄을 1 건 이상 냈는가?
   ...
   - `negative_control_missing`: 테스트 통과로 판정되는 조건에 `음성 대조:` 절이 빠졌는가?
   - `amendment_direction_uncomputed`: 집합형 amendment 의 direction 을 집합 비교 없이 자기신고했거나, 앵커 부재를 이유로 `unknown` 으로 적었는가?
   - `measure_premise_unrun`: 값이 면제인 조건의 측정 명령에서 준비 단계(읽을 경로 · `command -v` · 도구를 숨기는 전제 · 임시 사본)를 봉인 전에 돌리지 않았는가?
   - `known_answer_missing`: 새로 짠 측정 스크립트가 0 이 아닌 값을 내는 조건에 `알려진 답:` 절이 빠졌거나, 기대값과 실제값이 다른 채 봉인했는가?
2. 각 항목에 대해 true/false 판정

### 8. 교차 진단 (6.6 봉인 전에 실행)
