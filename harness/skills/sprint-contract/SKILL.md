...
섹션과 항목은 지우지 못하지만, **각 항목의 측정은 이번 변경에 맞게 채운다.** 이번 변경과 무관한 항목(예: `commands.test` 가 이 변경이 만들지 않는 산출물만 검사하는 경우, UI 컴포넌트가 없는 변경의 `RE-01`)은 **`N/A (사유)`** 로 적는다 — 통과도 실패도 못 하는 빈 검사를 남기지 마라. 실측: 계약 피드백 최근 10 건의 교차 진단 메모에서 `RE-01` 5 · `RE-02` 4 · `DG-03` 4 · `AP-01` 3 회가 "아무것도 재지 않는 조건" 으로 지적됐고, 자기진단 `untestable_conditions` 가 10 건 중 8 건이었다.
