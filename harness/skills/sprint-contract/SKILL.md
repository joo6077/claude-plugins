...
- **변경 범위(scope) 조건에 `git diff` 를 자유 서술로 적지 마라 — 표준형 5 요소를 다 채워라.** (1) `Given:` 상태 전제 (2) 경로 한정 pathspec (3) 생성물 제외 pathspec (`':(exclude)*.g.dart'` 등) (4) "정확히 일치" 인지 "포함" 인지 (5) **상한 ref — `HEAD` 금지**. 그리고 **계약 작성 시점에 그 명령을 1 회 실행해 baseline 을 서술 섹션에 남겨라.** 미커밋 codegen 산출물이 diff 에 섞여 scope 조건이 깨진 것이 AR-01 재발 3 회의 직접 원인이다 (E3 승급). contract-design-guide §Diff-Scope Oracle 표준형 참조
