...
**(3) 리터럴 환경값 금지 (environment literal)** — 조건 본문에 그 시점의 환경값을
**리터럴로 박지 마라.** 박힌 값은 환경이 바뀌는 순간 조건을 거짓으로 만들고, 평가자는
조건이 틀렸는지 구현이 틀렸는지 구별할 수 없다. 이것이 태그 `측정-환경-오염` 의 작성 단계
형태이며, 실측에서 3 회 반복된 패턴이다.
...
**(4) 미변경·부재 조건은 선언 지점을 한정해 재라 (unchanged / absence)** — "이 변경이 X 를
바꾸지 않았다" 를 **diff 텍스트 전체에 대한 리터럴 매칭**으로 재지 마라. 그런 oracle 은 "값이
...
부정형 oracle 은 서술이 늘어날수록 오탐이 늘어난다.

> **실제 발생 사례 (harness `amend-direction-baseline-case`, 2026-09-08)**: 조건 AR-02 의 측정식이
> `grep -cE '^\+.*v5\.[45]'` 였다. 커밋 `329e47c` 는 버전을 올리지 않았고 헤더에 "v5.4 는 다른
> 브랜치가 선점" 이라는 **순수 서술**만 넣었는데 count=1 로 FAIL 했다. 평가자 Improvement 원문:
> *"측정을 `## 스키마 버전` 섹션의 `현재: **v5.X**` 라인이 diff 에서 바뀌었는지로 한정"*.

**(5) 범위 추출 명령은 적은 뒤 한 번 실행해 줄 수를 확인하라** — 조건이 "그 섹션 안에서" 를
요구하면 섹션을 자르는 명령이 곧 oracle 의 일부다. 그 명령이 **아무것도 못 자르면** 조건은
"빈 곳에서 토큰을 찾는" 상태가 되고, 부정형 조건이면 **항상 통과**한다.

**awk 범위형 `/시작/,/종료/` 를 쓰지 마라.** 시작 줄이 종료 패턴에도 매치되면 범위가 그 줄에서
즉시 닫힌다. 같은 헤딩 계층을 경계로 쓰는 마크다운 문서에서는 **항상** 이렇게 죽는다.

같은 문서·같은 섹션에 두 형태를 돌린 결과다. 범위형은 헤더 1 줄, 플래그형은 섹션 본문까지 돌려준다.

```bash
awk '/^## 이 킷이 사실로 말하지 않는 것/,/^## /' README.md | wc -l
```

출력: `1`

```bash
awk '/^## 이 킷이 사실로 말하지 않는 것/{f=1;print;next} f&&/^## /{exit} f' README.md | wc -l
```
...
> **실제 발생 사례 (howto-kit `changelog-feeds`, 2026-09-09)**: 조건 AR-03 이 범위형을 썼다가
> 헤더 1 줄만 측정하는 상태로 봉인됐다. 구현 중 발견해 사이드카 `AM-01` 로 플래그형에 교체했고,
> `amend_direction_oracle` 로 `narrowing measured_removed=0 measured_added=5` 를 계산해 기록했다.
...
0 기대 조건에 **대상 파일을 열거하라.** "`.md` 5개" 처럼 개수만 적으면 다음 평가자가 같은 집합을
만들 방법이 없고, 범위가 나중에 늘면 더한 파일이 조용히 검사에서 빠진다.

실측(2026-09-23): `DG-02` 가 대상을 "`.md` 5개" 로 적었는데 그 스프린트가 실제로 고친 마크다운은
6 개였다. 나중에 범위에 더한 1 개가 검사에서 빠졌고, 그 파일에 경고가 생겨도 측정은 0 을 냈을
것이다. 교차 진단이 6 개 전부 다시 재서 값이 같음을 확인해 판정은 살았지만, 살린 것은 운이다.

개수는 열거한 목록을 세어 얻게 한다. 그러면 범위가 늘어도 조건 줄을 손댈 일이 없다 —
§계약 봉인의 "조건 줄이 가리키는 산문" 문제와 같은 뿌리다.

#### 취지가 서술 부재라면 낱말 하나로 재지 마라 (2026-09-23 추가)

조건의 취지가 "그런 지시·서술이 **없다**" 라면, 특정 낱말 하나가 0 건인 것으로 재면 안 된다.
그 낱말을 안 쓰고 같은 지시를 적으면 0 이 나오기 때문이다. 반대 방향도 위험하다 — 재려는 낱말이
**다른 문맥에서 이미 쓰이고 있으면** 아무것도 안 고쳐도 1 이상이 나와 통과한다.

실측(2026-09-23) 두 방향 다 겪었다.

- 없음 쪽: `ER-01` 이 "평가자가 교차 진단을 직접 한다는 서술 0 건" 을 `general-purpose` 글자
  하나로 쟀다. 그 글자 없이 "Agent 도구로 띄워라" 라고 적으면 통과한다. 이번엔 우연히 통과했다
- 있음 쪽: 어떤 조건이 "`산문`" 이라는 낱말 1 건 이상을 요구했는데, 그 낱말은 이미 대상 파일에
  6 건·2 건 있었다 (다른 문맥). 아무것도 안 고쳐도 통과하는 조건이었다 — 봉인 전 교차 진단이 잡았다
...
- 사용자 승인 직후 계약을 **봉인**한다: 조건 체크박스 줄만 정규화 해시해 frontmatter
  `conditions_digest` 와 `locked_at` 에 기록한다 (계산식은 스키마 §계약 봉인). 조건 아래 들여쓴
  측정 · 음성 대조 · 픽스처 줄은 `measurement_digest` 로 함께 잠근다 (v5.6) — 측정 줄은 들여써 적어야 잠긴다.
