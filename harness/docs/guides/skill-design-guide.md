...
| 12 | **절차 안내형** | 사람이 외부 UI·콘솔을 조작하도록 화면→메뉴→항목→값→확인까지 끊지 않고 안내 | howto, setup-guide |

> **현재 12 종.** 1~9 는 Anthropic 공식 분석 패턴, **10~12 는 본 레포 운영 경험에서 추가** — 10 은 긴 세션의 toll/network/output_token 한계로 truncation 이 잦은 환경에서 lifecycle 스킬이 별도 카테고리로 식별된 것(2026-05 /insights), 11 은 탐색이 발산해 산출물이 수십 개로 불어나고 변주끼리 구별되지 않는 실패가 반복 관측된 것(2026-08 /insights · 글로벌 REJECT `UI-04`), 12 는 아래 근거로 2026-09 에 추가한 것이다. 이 목록을 체크리스트로 사용하여 팀에 아직 없는 스킬 유형을 점검하라.
>
> **개수를 이 절의 제목이나 다른 문서의 인용 문구에 박지 마라.** 표가 9 → 11 로 늘었을 때 제목과 레포 내 인용 7 곳이 따라오지 않아 "9가지"라고 적힌 채 11 행인 상태가 오래 남았다. 개수는 이 주석에서만 말한다.
>
> **유형 11 에 속하는 스킬은 §5.6 Variant Budget 조항을 반드시 따른다.** 다른 유형과 달리 산출물 개수 자체가 실패 모드이기 때문이다.
>
> **유형 12 를 별도 슬롯으로 둔 근거 (2026-09).** 사람이 외부 UI 를 조작하도록 안내하는 스킬이 서로 다른 두 킷에 독립적으로 **4 개** 생겼다 — `howto-kit` 의 `howto` · `howto-doc` · `howto-audit`, 그리고 `onboarding-kit` 의 `setup-guide`. 기존 유형 중 정의상 맞는 슬롯이 없었다: 유형 1(라이브러리 레퍼런스)은 **코드 API** 사용법이고, 유형 8(런북)은 **장애 트리거**다.
>
> 슬롯을 정당화하는 것은 개수가 아니라 **고유 실패 모드**다. 유형 11 이 산출물 개수 때문에 별도인 것처럼, 유형 12 는 **입도 부족을 날조로 메우는 것**이 고유 실패다. "설정에서 활성화하세요" 수준의 안내를 자세히 쓰라고 요구하면, 모델은 확인하지 않은 메뉴 이름과 클릭 경로를 지어내서 요구를 충족시킨다 — 자세해졌는데 틀린 상태가 된다. 다른 어떤 유형에도 이 경로는 없다. 그래서 이 유형의 스킬은 **출처 등급 표기와 말단 액션 입도 규칙을 함께** 갖춰야 하며, 참조 구현은 `howto-kit` 이다 (harness 가이드는 특정 킷의 규칙을 강제하지 않는다 — 구현을 가리킬 뿐이다).
...
3. **검증 불가 시 `[미검증]` 에 네 칸을 붙인다.** 인프라 부재·도구 미설치·런타임 미실행으로 검증이 불가하면 조용히 넘기지 말고 해당 항목에 `[미검증]` 을 달고 아래 네 칸을 채운다.
   ...
   - **시도한 우회** — 하나 이상과 그 결과. 칸을 비우지 않는다. `없음` 은 `없음 — 계약에 대체 검증 단계가 없음` 한 가지만 쓴다 (`qa-evaluator.md` 규칙 11 (2) 가 계약 결함 기록으로 받는다). 계약에 대체 검증 단계가 있는데 건너뛰었으면 `없음` 으로 적어도 `INVALID` 다. 작업 자체를 못 한다고 할 때는 하나 이상이어야 한다
   - **통제 불가 사유** — 한 문장
...
#### 0 이 기대값인 검증의 양성 대조 — 스킬이 스스로 세운다

> **현재 등급: E2** (§3.7 등급 원장 참조)

위 5 조항이 "증거를 남겨라" 를 말한다면 이 항목은 **그 증거가 유효한가**를 말한다. 스킬이
"매치 0 건 · 오류 0 건 · 빈 출력 · 차이 없음" 을 통과 근거로 쓸 때, 그 0 이 **의도된 0** 인지
**측정이 죽어서 나온 0** 인지 구별하지 못하면 증거 블록은 있으나 마나다.

0 을 통과로 읽기 전에 셋을 남긴다.

1. **명령 성공** — "없음" 과 "실패" 를 종료 코드로 가른다 (`grep` 은 1 = 없음, 2 = 오류).
   `2>/dev/null` · `|| true` · 파이프가 오류를 삼키면 그것을 빼고 다시 돌린다
2. **대상 수** — 읽은 파일·줄·기록 수가 0 보다 크다. 대상이 0 이면 아무것도 검사하지 않은 것이다
3. **양성 대조** — 같은 측정이 **알려진 나쁜 예에서 1 이상**을 낸다. 기존 기록에서 찾거나,
   없으면 임시 사본을 만들어 확인한다 (대상 파일은 건드리지 않는다)

실측으로 잡힌 형태들이다. 기록 형식에 없는 문자열을 세는 조건이 항상 0 이라 통과했고
(2026-09-19), 경로 패턴에 `^\./` 를 붙였는데 이 환경의 `grep` 이 그 접두를 안 붙여 조용히
0 이 났고(2026-09-22), 셸 변수를 따옴표 없이 펼쳐 파일을 못 찾은 오류를 `2>/dev/null` 이
삼켜 0 으로 보였다(2026-09-22). 셋 다 "0 = 통과" 로 읽혔다.

**Cross-Surface Parity:** 본 원칙은 §11 parity 표 15 번째 항목의 **스킬(생성) 측 짝**이다.
