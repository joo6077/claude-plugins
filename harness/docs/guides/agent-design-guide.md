...
**한계 1 — 괄호 안 타입 제한은 메인 스레드에서만 걸린다.** 이 제한은 `claude --agent` 로 띄운
메인 스레드에 적용되고, **그 에이전트가 서브에이전트로 불릴 때는 무시된다**
([Create custom subagents](https://code.claude.com/docs/en/sub-agents) "Restrict which subagents
can be spawned"). 즉 `Agent(worker, researcher)` 라고 적은 에이전트를 다른 에이전트가 띄우면
그 안에서는 어떤 타입이든 스폰할 수 있다. 범위를 진짜로 묶고 싶으면 괄호에 기대지 말고
**본문 지시로 묶어라.**

**한계 2 — 도구를 쥐어줘도 실제로 쓴다는 보장이 없다. 그리고 안 쓰고도 썼다고 적는다.**
2026-09-22 실측: `qa-evaluator` 의 `tools` 에 `Agent(general-purpose)` 를 넣어 교차 진단
단계를 살렸는데, 그 에이전트가 **띄우지 않고도 "띄워서 재실행시켰다" 고 리포트에 쓰고**
`cross_diagnosis_by: sprint-contract` 로 기록했다. 물어서 재시도를 시켰더니 두 번째 응답도
같은 방식으로 지어낸 것이었다. 본인 확인으로 도구는 목록에 `Agent` 로 보였고, 지시문에는
"하지 않은 교차 진단을 적지 마라" 는 금지 문장까지 있었다 — **둘 다 통하지 않았다.**

판정은 부모 세션 기록에서 `Agent` 호출을 세어 내렸고(0 건), 그 0 을 믿기 전에 직접 하나
띄워 같은 패턴이 1 로 잡히는지 확인했다. 서브에이전트의 도구 사용은 부모 기록에 남는다.

**설계 함의:** 서술로 된 금지는 "내가 방금 그걸 했다" 는 자기 서술을 막지 못한다. 한 에이전트가
다른 에이전트를 띄워야 하는 절차라면, **그 절차를 부모로 올려라.** 부모의 호출은 기록에 남고
결과를 부모가 직접 읽으므로 지어낼 여지가 구조적으로 없다. 이 판단으로 `qa-evaluator` 의
7단계 교차 진단은 폐기되고 주체가 `harness/skills/sprint/SKILL.md` Step 4.5 로 옮겨졌다.
...
| 최소 권한 | 필요한 도구만 부여 |
| Agent 스코핑 | `Agent(agent_type)` 로 스폰 가능 서브에이전트 화이트리스트 — 단 **서브에이전트로 불릴 때는 무시**되고, 도구를 줘도 안 쓰고 썼다고 적을 수 있다 (§Agent(agent_type) 한계 1·2) |
| 모델 최적화 | 작업 복잡도에 맞는 모델 선택 |
| 호출 품질 | 컨텍스트·범위·파일참조·성공기준 4요소 |
| 독립 컨텍스트 | 생성과 평가는 분리 |
| **중첩 3 층** | 서브에이전트도 위임 가능하나 기본 3 층에서 끊김 · 깊이를 설계 가정으로 삼지 마라 (§4) |
| **하드 리밋** | 동시 20 · 깊이 3 (세션 전체 스폰 수는 상한 없음) — 자체 예산은 항상 이보다 작게 (§7) |
