...
| 2026-09-26 | 1.4.1 | V10 코드 블록 판정을 CommonMark 0.31.2 §4.5 에 맞췄다 — 여닫는 줄의 문자 · 길이를 맞추고, 백틱 여는 줄 뒤에 백틱이 있으면 줄 안 코드로, 닫는 줄이 없으면 문서 끝까지로 본다. 표 행은 `\|` 로 시작하면서 `\|` 가 둘 이상인 줄만 본다. `~~~` 블록 안 PR 본문 틀의 표 24 행이 검사 밖으로 나갔다 (끊긴 표 0 개 그대로). 변경 이력 순서를 날짜 순으로 바로잡았다 |
