...
**코드 블록 판정은 CommonMark 0.31.2 §4.5 를 따른다** (1.4.1). 닫는 줄은 여는 줄과 같은 문자(백틱 또는 `~`)이면서
길이가 같거나 길고, 뒤에 공백만 온다. 백틱으로 여는 줄의 뒤쪽 글에 백틱이 있으면 줄 안 코드라 블록이 아니다. 닫는 줄이
없으면 문서 끝까지가 블록이다. 백틱 3 개로 시작하는 줄마다 켜고 끄기만 뒤집던 1.4.0 까지의 판은 백틱 4 개 블록 안의
백틱 3 개 블록 · `~~~` 블록 · 줄 안 코드로 시작하는 줄에서 끊긴 표를 잘못 잡거나 놓쳤다. 그 결과 카이젠 스킬 넷의
`pr-template.md` 가 `~~~markdown` 블록에 담은 PR 본문 틀의 표 24 행은 코드라 검사 밖이다 (그중 끊긴 표는 0 개였다).

**`|` 가 두 개 이상인 줄만 표 행으로 본다** — 왼쪽 공백을 벗겨 `|` 로 시작하는 줄 가운데서 고른다. 목록 안에서 `|` 로
시작하는 보통 문장을 잘못 잡지 않게 한다. 줄 앞 조건을 빼면 `argument-hint: "<a> | <b>"` 같은 frontmatter 줄이 표 행이
된다. 앞 `|` 하나만 쓰는 한 칸짜리 표(`| a`)는 표 행으로 보지 않는다 — 레포에 0 개다.

**4 칸 들여쓴 코드 블록은 판정하지 않는다** — 목록 안에 들여쓴 표와 가르려면 목록 문맥을 따라가야 한다. 레포에 그런
블록 안 `|` 줄은 0 개다.

**규격과 다르게 단순화한 곳이 셋 더 있다** — 목록 · 인용 문맥을 따라가지 않으려고 줄였다. 레포 254 파일에서 판정 차이는
0 개다 (2026-09-26 교차 진단이 markdown-it · micromark 두 구현과 대조했다). ① 여닫는 줄의 들여쓰기 칸 수를 보지 않는다 —
목록 안 블록을 알아보려고 왼쪽 공백을 모두 벗기므로 4 칸 이상 들여쓴 백틱 줄도 여닫는 줄이 된다. ② 닫는 줄이 없으면
규격은 그 블록을 감싼 목록 · 인용이 끝나는 곳까지를 블록으로 보지만, 이 검사는 문서 끝까지로 본다. ③ 인용(`>`) 안의
코드 블록은 알아보지 않는다 — 인용 안 표 행은 줄 앞이 `>` 라 원래 표 행으로 보지 않는다.
...
| 2026-09-26 | 1.4.1 | V10 코드 블록 판정을 CommonMark 0.31.2 §4.5 에 맞췄다 — 여닫는 줄의 문자 · 길이를 맞추고, 백틱 여는 줄 뒤에 백틱이 있으면 줄 안 코드로, 닫는 줄이 없으면 문서 끝까지로 본다. 표 행은 `\|` 로 시작하면서 `\|` 가 둘 이상인 줄만 본다. `~~~` 블록 안 PR 본문 틀의 표 24 행이 검사 밖으로 나갔다 (끊긴 표 0 개 그대로). 규격과 다르게 단순화한 곳(여닫는 줄 들여쓰기 칸 수 · 목록 · 인용 경계)을 적었다. 변경 이력 순서를 날짜 순으로 바로잡았다 |
