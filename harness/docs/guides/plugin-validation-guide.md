...
**규격과 다르게 단순화한 곳이 있다** — 목록 · 인용 · HTML 블록 문맥을 따라가지 않으려고 줄였다. ① 여닫는 줄의 들여쓰기
칸 수를 보지 않는다 — 목록 안 블록을 알아보려고 왼쪽 공백을 모두 벗기므로 4 칸 이상 들여쓴 여닫는 줄(백틱 · `~`)도
여닫는 줄이 된다. ② 목록 · 인용 경계를 보지 않는다 — 닫는 줄이 없으면 문서 끝까지를 블록으로 보고, 목록 밖의 줄이 목록 안
블록을 닫기도 한다. ③ 인용(`>`) 안의 코드 블록은 알아보지 않는다 — 인용 안 표 행은 줄 앞이 `>` 라 원래 표 행으로 보지
않는다. ④ HTML 블록(`<!-- … -->` 등) 안의 백틱 줄도 여닫는 줄로 본다. ⑤ 닫는 줄 뒤의 공백 판정이 규격(공백 · 탭)보다
넓어 전각 공백도 공백으로 본다. 레포 254 파일에서 **끊긴 표 판정은 규격과 같다** — 코드 블록 줄 판정이 다른 곳은
`rust-kit/skills/rust-feature/SKILL.md` 102–104 행의 인용 안 블록 하나이고 그 안 표 행은 0 개다 (2026-09-26 교차 진단이
markdown-it · micromark 두 구현과 대조했다).
