...
- **조건 줄이 가리키는 산문을 고치면 개정 파일에 남긴다** (2026-09-23 추가). 봉인은 조건 줄만
  덮으므로 산문 편집은 `SEAL_OK` 를 유지한다. 그런데 조건이 "위 N 개 경로" · "아래 목록" 처럼
  산문을 가리키면 **그 산문은 사실상 조건의 일부**다. 산문을 고쳐 통과 집합이 달라지면 조건 문구를
  고친 것과 결과가 같다. 그러니 조건이 참조하는 산문을 고칠 때는 `sprint-amendments-<slug>.md` 에
...
위 두 규칙(산문 편집을 개정에 남기기 · 조용한 재봉인 금지)은 **대조할 원문이 있어야** 성립한다.
봉인 직후 계약 파일만 단독으로 커밋해 그 원문을 git 에 남긴다. 절차는
`harness/skills/sprint-contract/SKILL.md` Step 6.7 이 정의한다.

실측(2026-09-24, 계약 3 건): 봉인 뒤 2~6 분 지나 **구현 파일과 함께** 커밋됐다 (그 커밋에 담긴
파일 20 · 10 · 5 개). git 이 가진 가장 오래된 판이 이미 구현 뒤 상태라 그 사이 무엇이 바뀌었는지
증명할 길이 없었다. 계약 68 여 개 중 12 개는 추적조차 되지 않았다.

- 커밋에 **계약 파일 하나만** 담는다. 구현 파일이 섞이면 "봉인 시점 원문" 이라는 성질이 사라진다.
  새 파일은 `git add` 가 먼저 필요하고(추적 안 된 경로는 `git commit -o` 만으로는 죽는다),
  그래도 `-o` 를 붙여 다른 세션이 스테이징한 것을 삼키지 않는다
- **전용 가지에서 한다.** `main` 이 보호돼 있으면 직접 밀어 넣을 수 없다
- **병합할 때 스쿼시를 쓰지 마라.** 여러 커밋을 하나로 합치면 봉인 커밋이 `main` 기록에서
...
변경 범위 조건이 `.harness/` 를 재야 할 때, 이번 스프린트의 산출물 파일명을 화이트리스트로
열거하는 형태를 쓰지 마라. **여러 스프린트를 한 브랜치에 쌓으면 앞 스프린트의 status 전환이 항상
뒤 스프린트 구간에 들어온다.** 앞 스프린트가 APPROVE 되면 평가자가 그 계약의 `status` 를
`done` 으로 바꾸고, 그 한 줄이 뒤 스프린트의 화이트리스트에 없어 조건이 FAIL 한다.

실측(2026-09-23): 그 때문에 개정 1 건과 사용자 승인이 강제됐고, 다음 회차에서 같은 함정이
또 나올 구조였다. 재려던 것은 "과거 기록을 고치지 않았다" 인데 status 전환은 기록을 고치는 것이
아니라 기록이 스스로 완료를 표시하는 것이다.

**권장 형태** — status 전환과 조건 변조를 구별하는 검사를 쓴다.
`sprint-contract*.md 에 verify_seal 을 돌려 SEAL_BROKEN 이 0 개` 인지 본다.
...
##### 커밋 구간 상한 — `HEAD` 를 쓰지 마라 (v5.4 추가)

`<base>..HEAD` 로 쓰면 상한이 **평가 시점마다 움직인다.** 봉인 이후 무관한 작업이 브랜치에
들어오는 순간 그 파일이 화이트리스트 위반으로 잡히고, 스프린트는 자기가 만들지도 않은 변경
때문에 FAIL 한다. 상한은 **이 스프린트가 소유한 ref** 여야 한다.

실측 (`mcp-dynamic-pull-collision`, 같은 결함 2 회): SC-02 가 `1752e9a..HEAD` 로 서버 `lib/`
화이트리스트를 재는데, 봉인 뒤 병합된 별개 버그 수정(0.14.1)이 화이트리스트 밖 파일을 고쳐
SC-02 가 새로 FAIL 이 됐다. 스프린트 머지 커밋까지로 좁히자 4 경로 전부가 원 화이트리스트
안에 들어왔다 — 조건도 구현도 옳았고 상한만 틀렸다.

- 계약에 **상한을 해석하는 명령**을 적는다. 작성 시점에는 머지 커밋 해시를 모르므로 해시를
  박지 말고, 슬러그로 해석되는 형태를 쓴다.
...
sprint_head() {  # sprint_head <slug>
  m=$(git log --merges --format=%H --grep="Merge sprint/$1" -1)
  [ -n "$m" ] && { echo "$m"; return 0; }
  git rev-parse --verify -q "sprint/$1" && return 0
  echo "UNRESOLVED sprint/$1" >&2; return 1
}
```
...
`[미실측]` 을 붙이고, 실측으로 값을 확정하기 전에는 `conditions_digest` 를 계산하지 않는다.
`[미실측]` 은 봉인 시점에 계약에 남아 있으면 안 되는 임시 표기다 — 평가 시점의 도구 부재를
기록하는 `[미검증]` 과 다르다.

실측 배경 (2026-09-07 `mcp-dynamic-pull-collision`): RS-01 이 BEFORE 오라클로
«`summary.totalTools` 가 `0`» 을 잠갔는데 그 값은 작성 시점에 측정된 적이 없었다. 결함이 실제로
내는 값은 `37` 이었고 (`0` 은 VM 연결 **전**의 값이라 결함과 무관했다), 결함의 진짜 서명은 전혀
다른 관측 — `tools/list` 에서 서버 정적 도구가 앱 스텁으로 조용히 교체되는 것 — 이었다. 결함이
실재하는데도 조건이 구조적으로 PASS 할 수 없는 상태였다.

**이 실패는 개정으로만 빠져나갈 수 있고, 그 개정은 direction 계산상 반드시 `relaxing` 이 된다**
— 원 조건의 PASS 집합이 공집합이므로 무엇을 넣어도 집합이 늘어난다. 즉 미실측 수치 하나가
사용자 승인을 강제하는 amendment 를 만들어 낸다. 개정 오라클이 원 오라클보다 날카로워도
`narrowing` 으로 자기신고할 수 없다 (§Amendment 사이드카 축 1).

- **적용 대상**: 값을 고정하는 조건 (`== 0` · `>= 1500` · `정확히 N 행`) 과 열거 집합.
...
측정의 기대값이 **0 · 없음 · 빈 출력**인 조건은 그 0 이 "검사했는데 없다" 인지 "검사가 죽어서 0" 인지 구분되지 않는다. 그런 조건에는 `양성 대조:` 절을 넣는다.

- **무엇을 적나** — 같은 측정 명령이 **1 이상을 내는** 알려진 나쁜 예. 출처 순서: 같은 형식의 기존 기록(다른 파일 · 다른 세션) → 계약 작성자가 만들 임시 사본. 경로와 기대 건수를 함께 적는다
- **언제 재나** — **봉인 전에 1 회 실측**한다. 실측하지 않은 대조는 `[미실측]` 이며 §미실측 오라클 봉인 금지 · Step 6.5 (5) 가 막는다
- **명령 성공도 함께** — `2>/dev/null` · `|| true` 로 오류를 삼키는 측정은 "없음" 과 "실패" 를 못 가른다. 종료 코드로 가르는 형태로 적는다 (grep 은 1 = 없음, 2 = 오류)
- **평가자 쪽 대응** — `harness/agents/qa-evaluator.md` 규칙 10 이 이 절을 첫 출처로 읽는다. 절이 없으면 평가자가 직접 대조를 만들며, 만들 수 없으면 그 조건은 `[미검증:INVALID]` 다
- **음성 대조와의 차이** — 음성 대조는 "구현을 무력화하면 이 측정이 FAIL 하는가"(테스트 통과 조건용), 양성 대조는 "문제가 있으면 이 측정이 1 이상을 내는가"(0 기대 조건용)다
...
> **왜 통합했나 (v5.3).** 이전 판은 평가자 집합 5 종과 작성 집합 6 종이 **교집합 `측정-중복`
...
개정의 앵커가 틀렸다는 이유로 REJECT 를 받았다면, 고친 뒤 재평가를 받을 수 있다. 다만 **뒤집는
근거는 구현자가 쓰지 않은 기록에서 확인돼야 한다.** 개정 문서의 서술만 다듬어 통과시키는 것은
봉인 뒤 계약을 고쳐 판정을 바꾸는 것과 같다.

**판정 기준은 하나다 — 구현자가 사후에 고칠 수 있는가, 그리고 고친 것이 드러나는가.** 기록의 **구조적 값**(시각 · 식별자 ·
해시 · 도구가 찍은 출력)은 고칠 수 없으니 근거가 되고, 그 안에 **사람이 쓴 서술**은 고칠 수
있으니 근거가 못 된다. 같은 파일 안에서도 이 선으로 갈린다.

| 뒤집는 근거 | 인정 | 왜 |
| --- | --- | --- |
| 세션 기록의 시각 · `tool_use` 구조 · 도구가 찍은 출력 | 가능 | 구현자가 고칠 수 없다 |
| git 의 커밋 해시 · 커밋 시각 · diff | 가능 | 고치면 해시가 바뀌어 드러난다 |
| 평가자가 **직접 다시 돌려** 얻은 값 | 가능 | 구현자를 거치지 않는다 |
| **커밋 메시지 본문** | **불가** | git 기록이지만 구현자가 쓴 서술이다 |
| 개정 문서에 새로 쓴 설명·해명 | **불가** | 구현자가 쓴 글이 근거가 된다 |
| 계약·지침의 "실측(…)" 문장 | **불가** | 도구 출력을 구현자가 옮겨 적은 것이다 |
| **아직 원격에 밀어 올리지 않은 커밋** | **불가** | `commit --amend` · 재배치로 갈아도 드러나지 않는다. 밀어 올린 뒤 근거가 된다 |
| QA 피드백 파일 | **불가** | 평가자가 썼지만 구현자가 고칠 수 있는 평범한 레포 파일이다 |
| 계약의 `locked_at` | **불가** | 구현자가 손으로 적은 시각이다. `conditions_digest` 는 다시 계산해 확인되지만 이 값은 기준점이 없다 |

**행 이름을 "누가 썼나" 로 두면 갈린다.** 세션 기록 안의 대부분은 구현자 자신이 한 말이지만
그 **시각과 구조**는 못 고친다. 반대로 커밋 메시지는 git 기록인데 본문은 구현자가 쓴 글이다 —
실측(2026-09-24): 이번 세션의 커밋 메시지 하나가 "불릿 목록 뒤로 이동" 이라 적었으나 실제로는
헤딩 뒤에 불릿이 남아 사실과 달랐다. 교차 진단이 이 경계 흔들림을 짚었다.

평가자는 그 값을 **직접 뽑아** 확인한다. 구현자의 서술을 인용하지 마라. 교차 진단이 독립으로
같은 값을 뽑으면 근거가 한 겹 더 굳는다.

실측(2026-09-23): 개정 A-01 이 동의 시각을 짐작해 적어 REJECT 를 받았고, 고칠 때 세션 기록의
...
- **v5.4 (2026-09-08)** — **커밋 구간 상한에서 `HEAD` 금지.** 실측 `mcp-dynamic-pull-collision`
  ...
  상한은 스프린트가 소유한 ref (머지 커밋 또는 브랜치 tip) 여야 하고, 계약이 그 해석 명령을
  적는다. Diff-Scope 표준형이 4 요소 → 5 요소가 된다. **미실측 오라클 봉인 금지.** 실측 `mcp-dynamic-pull-collision`:
  RS-01 이 «`totalTools` 가 `0`» 을 BEFORE 오라클로 잠갔으나 그 값은 작성 시점에 측정된 적이
  없었고, 결함이 내는 실제 값은 `37` 이었다. 원 조건의 PASS 집합이 공집합이 되어 개정이 유일한
  출구가 됐고, 그 개정은 집합 비교상 필연적으로 `relaxing` 이라 사용자 승인을 강제했다 —
  잘못 쓰인 계약 한 줄이 사용자 시간을 소비하는 구조다. 값을 고정하는 조건은 봉인 전에 1 회
  실측하고, 미실측이면 `[미실측]` 표기 후 봉인을 보류한다 (구현이 만들어 낼 값은 면제).
