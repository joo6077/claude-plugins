...
block_deleted() {  # block_deleted <개수> <이름 목록> <작업 폴더 삭제 목록>
  block "삭제 $1 개가 실린 커밋을 막았다 (기준 $limit 개 초과${3:+ · 같은 명령의 git add 나 -a 가 올릴 작업 폴더 삭제 포함})." "상위 폴더:
$(top_dirs "$2")
확인: git diff --cached --stat --diff-filter=D"
}

parse_commit_args() {
  c_all=0 c_incl=0 c_paths=0 c_dry=0
  local a take=0 after_dd=0 k ch
  for a in "$@"; do
    if [ "$take" = 1 ]; then take=0; continue; fi
    if [ "$after_dd" = 1 ]; then c_paths=$((c_paths + 1)); continue; fi
    case $a in
      --) after_dd=1 ;;
      --all) c_all=1 ;;
      --include) c_incl=1 ;;
      --dry-run) c_dry=1 ;;
      --pathspec-from-file=*) c_paths=$((c_paths + 1)) ;;
      --pathspec-from-file) c_paths=$((c_paths + 1)); take=1 ;;
...
check_commit_pre() {  # check_commit_pre <저장소 폴더> <GIT_INDEX_FILE 값> <commit 인자…>
  ...
  [ "$c_paths" -gt 0 ] && [ "$c_incl" = 0 ] && return 0
  g_dir=$d g_index=""
