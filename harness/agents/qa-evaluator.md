...
- 계약의 범위 조건이 쓰는 커밋 구간 `<base>..<상한>` 에서 지운 파일을 전부 뽑는다 — `git diff --name-status --diff-filter=D <base>..<상한>`. 커밋 구간에는 커밋하지 않은 삭제가 없으므로 `git status --porcelain` 줄의 앞 두 글자(상태 칸)에 `D` 가 있는 줄도 함께 뽑는다 (`grep -E '^(D.|.D) '` — 경로에 든 `D` 는 세지 않는다)
